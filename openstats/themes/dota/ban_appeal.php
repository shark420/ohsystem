...
<?php } else { ?>
	  <div><?=$lang["you_must_be"] ?> <a href="<?=$website?>?login"><?=$lang["logged_in"]?></a> <?=$lang["to_appeal"] ?></div>
<?php } ?>
	  <div class="padTop"></div>
	  <div><?=$lang["appeal_ban_date"]?> <?=date( $DateFormat, strtotime( $BanAppealDate ) )?></div>
	  <div><a href="<?=$website?>?u=<?=strtolower($BanAppealName)?>"><?=$BanAppealName?></a> <?=$lang["was_banned"]?> 
