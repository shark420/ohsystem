...
<?php
  if ( isset($_GET["uid"]) AND is_numeric($_GET["uid"]) AND isset($GamesData[0]["player"]) ) {
   ...
   <div class="padTop padBottom">
     <h2><?=$lang["game_history"]?> <a href="<?=$website?>?u=<?=(int)$_GET["uid"]?>"><?=$GamesData[0]["player"]?></a> <?=GetHeroIcon()?></h2>
   </div>
   <?php
  }
?>
<?=DisplayGameFilter($StartYear) ?>
  ...
  ?>
  <tr class="row">
	 <td width="240" class="padLeft font12">
	 <?=GetHeroIcon()?>
	 <a href="<?=$website?>?game=<?=$Games["id"]?>"><span class="font13 winner<?=$Games["winner"]?>"><?=$Games["gamename"]?></span></a>
