...
<h3 id="comments"><?=$lang["comments"]?> (<?=$CommentsData[0]["total_comments"]?>)</h3>
...
<?php } else { ?>
	    <span class="<?php if ( OS_is_admin() ) { ?>CommentAdmin<?php } ?>"><img src="<?=$Comment["avatar"]?>" alt="*" width="64" height="64" class="imgvaligntop" /> 
		<b><?=LinkUserProfile($Comment["username"], $Comment["website"])?></b>
		</span>, 
<?php } ?>
		<a name="c-<?=$Comment["id"]?>" href="<?=OS_HOME?>?post_id=<?=$Comment["post_id"]?>#c-<?=$Comment["id"]?>"><i><?=$Comment["date"]?></i> &raquo;</a>
...
<?php } ?> 
	<tr>
	  <th width="840" class="alignleft padLeft"><?=$lang["add_comment"]?></th>
	</tr>
	<tr>
	  <td class="padLeft padTop padBottom">
	     <?=os_commentForm()?>
