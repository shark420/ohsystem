...
class database {
   ...
   function getUserWins($username) {
	$sql = "SELECT COUNT(*) FROM ".OSDB_DP." as gp LEFT JOIN ".OSDB_GAMES." as g ON g.id=gp.gameid LEFT JOIN dotaplayers as dp ON dp.gameid=g.id AND dp.colour=gp.colour LEFT JOIN ".OSDB_DG." as dg ON g.id=dg.gameid WHERE LOWER(name) = LOWER('$username') AND ((winner=1 AND dp.newcolour>=1 AND dp.newcolour<=5) OR (winner=2 AND dp.newcolour>=7 AND dp.newcolour<=11)) AND gp.`left`/g.duration >= 0.8 LIMIT 1";
	...
	$sql = "SELECT COUNT(*) FROM gp as gp LEFT JOIN ".OSDB_GAMES." as g ON g.id=gp.gameid LEFT JOIN ".OSDB_DP." as dp ON dp.gameid=g.id AND dp.colour=gp.colour LEFT JOIN ".OSDB_DG." as dg ON g.id=dg.gameid WHERE name='$username' AND ((winner=2 AND dp.newcolour>=1 AND dp.newcolour<=5) OR (winner=1 AND dp.newcolour>=7 AND dp.newcolour<=11)) AND gp.`left`/g.duration >= 0.8  LIMIT 1";
