...
class CGamePlayerSortAscByPing {
    public: bool operator()(CGamePlayer * Player1, CGamePlayer * Player2) const {
...
m_EarlyDraw(false) {
    ...
    if (m_Map - > GetMapType() == "w3mmd") m_Stats = new CStatsW3MMD(this, m_Map - > GetMapStatsW3MMDCategory());
    else if (m_Map - > GetMapType() == "dota") m_Stats = new CStatsDOTA(this);
...
CGame::~CGame() {
    ...
    if (m_CallableGameAdd && m_CallableGameAdd - > GetReady()) {
        ...
        } else CONSOLE_Print("[GAME: " + m_GameName + "] unable to save player/stats data to database");
    ...
    for (vector < PairedPassCheck > ::iterator i = m_PairedPassChecks.begin(); i != m_PairedPassChecks.end();) {
        if (i - > second - > GetReady()) {
            ...
            if (Player) {
                ...
                } else if (Result == 2) SendChat(Player, "Error. Wrong Password");
                else if (Result == 3) SendChat(Player, "Error. You havent activated your password protection.");
                else if (Result == 4) SendChat(Player, "Successfully removed your password protection.");
                else SendAllChat("Something is wrong there.");
            }
...
*/
    ...
    for (vector < Pairedpm > ::iterator i = m_Pairedpms.begin(); i != m_Pairedpms.end();) {
        if (i - > second - > GetReady()) {
            ...
            CGamePlayer * Player = GetPlayerFromName(i - > first, true);
            if (Player) {
                if (Result == -1) SendChat(Player, "The Message has successfully stored.");
                else if (Result == 2) SendChat(Player, "Welcome [" + Player - > GetName() + "] You have [" + UTIL_ToString(Result) + "] Message(s). Check it out with the command: '!inbox'.");
            }
        ...
        if (i - > second - > GetReady()) {
            ...
            if (StatsPlayerSummary) {
                if (i - > first.empty()) {
                    ...
                    if (StatsPlayerSummary - > GetStreak() < 0) string Streak = "-" + UTIL_ToString(StatsPlayerSummary - > GetStreak());

                    SendAllChat(m_GHost - > m_Language - > HasPlayedGamesWithThisBot(StatsPlayerSummary - > GetPlayer(), UTIL_ToString(StatsPlayerSummary - > GetScore(), 0), UTIL_ToString(StatsPlayerSummary - > GetGames()), UTIL_ToString(StatsPlayerSummary - > GetWinPerc(), 2), Streak));
                } else {
                    CGamePlayer * Player = GetPlayerFromName(i - > first, true);
                    ...
                    if (Player) {
                        ...
                        string Streak = "";
                        if (sn < 0) string Streak = "-" + UTIL_ToString(sn);
                        else string Streak = UTIL_ToString(sn);

                        SendChat(Player, m_GHost - > m_Language - > HasPlayedGamesWithThisBot(Player - > GetName(), UTIL_ToString(StatsPlayerSummary - > GetScore(), 0), UTIL_ToString(StatsPlayerSummary - > GetGames()), UTIL_ToString(StatsPlayerSummary - > GetWinPerc(), 2), Streak));
                    }
                }
            } else {
                if (i - > first.empty()) SendAllChat(m_GHost - > m_Language - > HasntPlayedGamesWithThisBot(i - > second - > GetName()));
                else {
                    CGamePlayer * Player = GetPlayerFromName(i - > first, true);

                    if (Player) SendChat(Player, m_GHost - > m_Language - > HasntPlayedGamesWithThisBot(i - > second - > GetName()));
                }
            }
        ...
        if (i - > second - > GetReady()) {
            ...
            if (StatsPlayerSummary) {
                ...
                if (i - > first.empty()) SendAllChat(m_GHost - > m_Language - > HasntPlayedGamesWithThisBot(i - > second - > GetName()));
                    ...
                    if (Player) SendChat(Player, m_GHost - > m_Language - > HasntPlayedGamesWithThisBot(Player - > GetName()));
                }
            }
    ...
    for (vector < PairedStreakCheck > ::iterator i = m_PairedStreakChecks.begin(); i != m_PairedStreakChecks.end();) {
        if (i - > second - > GetReady()) {
            ...
            if (StatsPlayerSummary) {
                if (StatsPlayerSummary - > GetStreak() != 0) SendAllChat("[" + StatsPlayerSummary - > GetPlayer() + "] Current Streak: " + UTIL_ToString(StatsPlayerSummary - > GetStreak()) + " | Max Streak: " + UTIL_ToString(StatsPlayerSummary - > GetMaxStreak()) + " | Max LosingStreak: " + UTIL_ToString(StatsPlayerSummary - > GetMaxLosingStreak()));
                else SendAllChat("[" + StatsPlayerSummary - > GetPlayer() + "] Current Streak: -" + UTIL_ToString(StatsPlayerSummary - > GetLosingStreak()) + " | Max Streak: " + UTIL_ToString(StatsPlayerSummary - > GetMaxStreak()) + " | Max Losing Streak: " + UTIL_ToString(StatsPlayerSummary - > GetMaxLosingStreak()));
            } else SendAllChat(m_GHost - > m_Language - > HasntPlayedGamesWithThisBot(i - > second - > GetName()));
    ...
    for (vector < PairedINCheck > ::iterator i = m_PairedINChecks.begin(); i != m_PairedINChecks.end();) {
        if (i - > second - > GetReady()) {
            ...
            CGamePlayer * Player = GetPlayerFromName(i - > first, true);
            if (Player) {
                if (InboxSummary) SendChat(Player, "[" + InboxSummary - > GetUser() + "] " + InboxSummary - > GetMessage());
                else SendChat(Player, "Your Inbox is empty.");
            }
    ...
    for (vector < PairedSCheck > ::iterator i = m_PairedSChecks.begin(); i != m_PairedSChecks.end();) {
        if (i - > second - > GetReady()) {
            ...
            if (StatsPlayerSummary) {
                string Summary = m_GHost - > m_Language - > HasPlayedDotAGamesWithThisBot(i - > second - > GetName(), UTIL_ToString(StatsPlayerSummary - > GetGames()), UTIL_ToString(StatsPlayerSummary - > GetWins()), UTIL_ToString(StatsPlayerSummary - > GetLosses()), UTIL_ToString(StatsPlayerSummary - > GetKills()), UTIL_ToString(StatsPlayerSummary - > GetDeaths()), UTIL_ToString(StatsPlayerSummary - > GetCreeps()), UTIL_ToString(StatsPlayerSummary - > GetDenies()), UTIL_ToString(StatsPlayerSummary - > GetAssists()), UTIL_ToString(StatsPlayerSummary - > GetNeutrals()), UTIL_ToString(StatsPlayerSummary - > GetTowers()), UTIL_ToString(StatsPlayerSummary - > GetRax()), UTIL_ToString(StatsPlayerSummary - > GetAvgKills(), 2), UTIL_ToString(StatsPlayerSummary - > GetAvgDeaths(), 2), UTIL_ToString(StatsPlayerSummary - > GetAvgCreeps(), 2), UTIL_ToString(StatsPlayerSummary - > GetAvgDenies(), 2), UTIL_ToString(StatsPlayerSummary - > GetAvgAssists(), 2), UTIL_ToString(StatsPlayerSummary - > GetAvgNeutrals(), 2), UTIL_ToString(StatsPlayerSummary - > GetAvgTowers(), 2), UTIL_ToString(StatsPlayerSummary - > GetAvgRax(), 2));

                if (i - > first.empty()) SendAllChat(Summary);
                else {
                    CGamePlayer * Player = GetPlayerFromName(i - > first, true);

                    if (Player) SendChat(Player, Summary);
                }
            } else {
                if (i - > first.empty()) SendAllChat(m_GHost - > m_Language - > HasntPlayedDotAGamesWithThisBot(i - > second - > GetName()));
                else {
                    CGamePlayer * Player = GetPlayerFromName(i - > first, true);
    ...
    for (vector < PairedSS > ::iterator i = m_PairedSSs.begin(); i != m_PairedSSs.end();) {
        if (i - > second - > GetReady()) {
            ...
            if (i - > second - > GetType() == "betcheck" || i - > second - > GetType() == "bet") {
                if (i - > second - > GetType() == "betcheck") SendAllChat("[" + i - > second - > GetUser() + "] Current Points: " + Result);

                else if (i - > second - > GetType() == "bet") {
                    if (Result == "already bet") SendChat(Player, "You already bet");
                    else if (Result == "successfully bet") SendAllChat("User [" + i - > second - > GetUser() + "] bet [" + UTIL_ToString(i - > second - > GetOne()) + "] to win this game.");
                    else if (Result != "failed") SendChat(Player, "You shouldn't bet more points you got, you got currently [" + Result + " ] points");
                    else CONSOLE_Print("Betsystem have an issue here");
                } else if (Result == "not listed") SendChat(Player, "You need to play at least one game to bet here");
                else CONSOLE_Print("Betsystem has an issue here");
            }
            m_GHost - > m_DB - > RecoverCallable(i - > second);
            delete i - > second;
...
void CGame::EventPlayerDeleted(CGamePlayer * player) {
    ...
    if (m_GameLoading || m_GameLoaded) {
        ...
        for (vector < CDBBan * > ::iterator i = m_DBBans.begin(); i != m_DBBans.end(); ++i) {
            if (( * i) - > GetName() == player - > GetName()) m_DBBanLast = * i;
        }

        // if this was early leave, suggest to draw the game
        if (m_GameTicks < 1000 * 60) SendAllChat("Use !draw to vote to draw the game.");
        ...
        if (!m_SoftGameOver && m_Stats && m_GameOverTime == 0 && Team != 12 && m_GameTicks < 1000 * 60 * 7 && m_GHost - > m_EarlyEnd) {
            ...
            for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); i++) {
                if ( * i && !( * i) - > GetLeftMessageSent() && * i != player) m_NumLeavers--;
            }
...
bool CGame::EventPlayerBotCommand(CGamePlayer * player, string command, string payload) {
    ...
    if (m_GHost - > m_GarenaHostingOnly) {
        ...
        for (vector < string > ::iterator i = m_GarenaPermissions.begin(); i != m_GarenaPermissions.end(); ++i) {
            ...
            if (username == name) {
                ...
                Level = level;
                if (level == 0) LevelName = "Level 1 User";
                else if (level == 1) LevelName = "Level 2 User";
                else if (level == 2) LevelName = "Level 3 User";
                else if (level == 3) LevelName = "Safelisted User";
                else if (level == 4) LevelName = "Devoted User";
                else if (level == 5) LevelName = "Bot Moderator";
                else if (level == 6) LevelName = "Full Bot Moderator";
                else if (level == 7) LevelName = "Global Bot Moderator";
                else if (level == 8) LevelName = "Owner";
                else if (level == 9) LevelName = "Admin";
                else if (level == 10) LevelName = "Root Admin";
            }
        }
    }
    if (player - > GetSpoofed() && Level >= 5) {
        ...
        if ((m_Locked && Level > 8) || !m_Locked) {
            ...
            if ((Command == "only" || Command == "unallow" || Command == "disallow" || Command == "deniecountry") && !m_GameLoading && !m_GameLoaded) {
                if (Payload.empty() || Payload == "0" || Payload == "clear") {
                    ...
                    if (Command == "only") m_LimitCountries = true;
                    else m_DenieCountries = true;
                    ...
                    for (vector < string > ::iterator i = m_LimitedCountries.begin(); i != m_LimitedCountries.end(); i++) {
                        if (AllLimitedCountries.empty()) AllLimitedCountries = * i;
                        else AllLimitedCountries = AllLimitedCountries + ", " + * i;
                    }
                    ...
                    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); i++) {
                        ...
                        for (vector < string > ::iterator k = m_LimitedCountries.begin(); k != m_LimitedCountries.end(); k++) {
                            if ( * k == CC && m_DenieCountries) unallowedcountry = true;
                            if ( * k != CC && m_LimitCountries) unallowedcountry = true;
                        }
            ...
            //
                ...
                } else if (Payload == "off") m_GameNoGarena = false;
                else SendChat(player, "Error please use on/off as config settings");
            }
            ...
            //
                ...
                if (Matches == 0) {
                    ...
                    m_Balanced = false;
                } else SendChat(player, "Error. Found multiply matches for the name: " + Payload);
            }
            ...
            //
                ...
                if (!Payload.empty()) StatsUser = Payload;
            ...
            //
                ...
                SS << Payload;
                SS >> suser;
                if (SS.fail() || suser.empty()) SendChat(player, "Error, wrong input, use '!simulatechat user message'");
                else if (suser.size() < 3) SendChat(player, "Error, the name is to short, please add a valied name");
                else {
                    SS >> message;
                    if (!SS.eof()) {
                        getline(SS, message);
                        string::size_type Start = message.find_first_not_of(" ");
                        if (Start != string::npos) message = message.substr(Start);
                    }
                    if (message.length() > 100) SendChat(player, "Error, this message is to long, may choose a more shorten message.");
                    else if (message.empty()) SendChat(player, "Error, there is no message set.");
                    else {
                        CGamePlayer * LastMatch = NULL;
                        uint32_t Matches = GetPlayerFromNamePartial(suser, & LastMatch);
                        if (Matches == 0) SendChat(player, "Error. Found no match on the playername");
                        else if (Matches == 1) {
                            SendChat(player, "Successfully sent a simulated message");
                            SendAllChat((unsigned char) LastMatch - > GetPID(), message);
                        } else if (Matches > 1) SendChat(player, "Error. Found more than one match for this playername.");
                    }
                }
                return true;
            }
            ...
            //
                ...
                if (SS.fail() || Victim.empty()) CONSOLE_Print("[PP] bad input #1 to !TEMPBAN command");
                else if (Victim.size() < 3) SendChat(player, "Error. The name is too short, please add a valied name");
                else {
                    SS >> Amount;

                    if (SS.fail() || Amount == "0") CONSOLE_Print("[PP] bad input #2 to !TEMPBAN command");
                    else if ((UTIL_ToUInt32(Amount) > 3 && Level < 8) || UTIL_ToUInt32(Amount) > 10 && Level <= 10) SendChat(player, "You shouldn't add more than 3 penality points");
                    else {
                        SS >> Reason;
                        ...
                        if (!SS.eof()) {
                            ...
                            if (Start != string::npos) Reason = Reason.substr(Start);
                        }
                        if (!Reason.empty()) {
                            CGamePlayer * LastMatch = NULL;
                            uint32_t Matches = GetPlayerFromNamePartial(Victim, & LastMatch);
                            if (Matches == 0) SendChat(player, "Error. Found no match on the playername");
                            else if (Matches == 1) m_Pairedpenps.push_back(Pairedpenp(string(), m_GHost - > m_DB - > Threadedpenp(LastMatch - > GetName(), Reason, User, UTIL_ToUInt32(Amount), "add")));
                            else if (Matches > 1) SendChat(player, "Error. Found more than one match for this playername.");
                        } else SendChat(player, "Error. Please state a reason to punish someone");
                    }
                }
            }
            ...
            //
                ...
                if (Level >= 7) {
                    ...
                    if (m_GameLoaded) {
                        ...
                        if (Matches == 0) SendAllChat(m_GHost - > m_Language - > UnableToBanNoMatchesFound(Victim));
                        else if (Matches == 1) m_PairedBanAdds.push_back(PairedBanAdd(User, m_GHost - > m_DB - > ThreadedBanAdd(LastMatch - > GetServer(), LastMatch - > GetName(), LastMatch - > GetIP(), m_GameName, User, Reason, 0, "")));
                        else SendAllChat(m_GHost - > m_Language - > UnableToBanFoundMoreThanOneMatch(Victim));
                    } else {
                        CGamePlayer * LastMatch = NULL;
                        uint32_t Matches = GetPlayerFromNamePartial(Victim, & LastMatch);

                        if (Matches == 0) SendAllChat(m_GHost - > m_Language - > UnableToBanNoMatchesFound(Victim));
                        else if (Matches == 1) m_PairedBanAdds.push_back(PairedBanAdd(User, m_GHost - > m_DB - > ThreadedBanAdd(LastMatch - > GetJoinedRealm(), LastMatch - > GetName(), LastMatch - > GetExternalIPString(), m_GameName, User, Reason, 0, LastMatch - > GetCLetter())));
                        else SendAllChat(m_GHost - > m_Language - > UnableToBanFoundMoreThanOneMatch(Victim));
                    }
                } else SendChat(player, "You dont have access to run this command, please use '!tban'");
            }
            ...
            if ((Command == "tempban" || Command == "tban" || Command == "tb") && !Payload.empty() && !m_GHost - > m_BNETs.empty()) {
                ...
                if (SS.fail() || Victim.empty()) CONSOLE_Print("[TEMPBAN] bad input #1 to !TEMPBAN command");
                else if (Victim.size() < 3) SendChat(player, "Error. The name is too short, please add a valied name");
                else {
                    SS >> Amount;

                    if (SS.fail() || Amount == 0) CONSOLE_Print("[TEMPBAN] bad input #2 to !TEMPBAN command");
                    else {
                        SS >> Suffix;

                        if (SS.fail() || Suffix.empty()) CONSOLE_Print("[TEMPBAN] bad input #3 to autohost command");
                        else {
                            uint32_t BanTime = 0;
                            ...
                            if (ValidSuffix) {
                                ...
                                if (m_GameLoaded) {
                                    ...
                                    if (Matches == 0) SendAllChat(m_GHost - > m_Language - > UnableToBanNoMatchesFound(Victim));
                                        ...
                                        if (Level >= 7 || (Level == 5 || Level == 6) && ((Suffix == "hour" || Suffix == "hours" || Suffix == "h") || ((Suffix == "days" || Suffix == "d" || Suffix == "day") && Amount <= 5))) m_PairedBanAdds.push_back(PairedBanAdd(User, m_GHost - > m_DB - > ThreadedBanAdd(LastMatch - > GetServer(), LastMatch - > GetName(), LastMatch - > GetIP(), m_GameName, User, Reason, BanTime, "")));
                                        else SendChat(player, "You have no permission to ban this player.");
                                    } else SendAllChat(m_GHost - > m_Language - > UnableToBanFoundMoreThanOneMatch(Victim));
                                } else {
                                    CGamePlayer * LastMatch = NULL;
                                    uint32_t Matches = GetPlayerFromNamePartial(Victim, & LastMatch);
                                    ...
                                    if (Matches == 0) SendAllChat(m_GHost - > m_Language - > UnableToBanNoMatchesFound(Victim));
                                        ...
                                        if (Level >= 7 || (Level == 5 || Level == 6) && ((Suffix == "hour" || Suffix == "hours" || Suffix == "h") || ((Suffix == "days" || Suffix == "d" || Suffix == "day") && Amount <= 5))) m_PairedBanAdds.push_back(PairedBanAdd(User, m_GHost - > m_DB - > ThreadedBanAdd(LastMatch - > GetJoinedRealm(), LastMatch - > GetName(), LastMatch - > GetExternalIPString(), m_GameName, User, Reason, BanTime, LastMatch - > GetCLetter())));
                                        else SendChat(player, "You have no permission to ban this player");
            ...
            //
                ...
                if (Payload.empty() || Payload == "off") {
                    ...
                    if (SS.fail() || Interval == 0) CONSOLE_Print("[GAME: " + m_GameName + "] bad input #1 to announce command");
                    else {
                        if (SS.eof()) CONSOLE_Print("[GAME: " + m_GameName + "] missing input #2 to announce command");
                        else {
                            getline(SS, Message);
                            string::size_type Start = Message.find_first_not_of(" ");
            ...
            //
                ...
                if (Level >= 7) {
                    if (Payload.empty()) Payload = "Leaver";
                ...
                } else SendChat(player, "Error, you have no permission to execute this command. Please use '!tbl <reason>' instead of banlast.");
            }
            ...
            //
                ...
                if (VictimLevel <= 1 || Level >= 9) m_PairedBanAdds.push_back(PairedBanAdd(User, m_GHost - > m_DB - > ThreadedBanAdd(m_DBBanLast - > GetServer(), m_DBBanLast - > GetName(), m_DBBanLast - > GetIP(), m_GameName, User, Payload, 432000, "")));
                else SendChat(player, "You have no permission to ban this player");
            }
            ...
            //
                ...
                if (!Payload.empty()) {
                    ...
                    if (Matches == 0) SendAllChat(m_GHost - > m_Language - > UnableToCheckPlayerNoMatchesFound(Payload));
                    else if (Matches == 1) {
                        uint32_t CLevel = 0;
                        string CLevelName;
                        for (vector < CBNET * > ::iterator i = m_GHost - > m_BNETs.begin(); i != m_GHost - > m_BNETs.end(); ++i) {
                    ...
                    } else SendAllChat(m_GHost - > m_Language - > UnableToCheckPlayerFoundMoreThanOneMatch(Payload));
                } else SendAllChat("[" + User + "] (P: " + (player - > GetNumPings() > 0 ? UTIL_ToString(player - > GetPing(m_GHost - > m_LCPings)) + "ms" : "N/A") + ") (F: " + player - > GetCLetter() + ") (Role: " + (LevelName.empty() ? "unknown" : LevelName) + ") (SpoofChecked: " + (player - > GetSpoofed() ? "Yes" : "No") + ") (Realm: " + (player - > GetSpoofedRealm().empty() ? "N/A" : player - > GetSpoofedRealm()) + ")");
            }
            ...
            //
                ...
                while (!SS.eof()) {
                    ...
                    if (SS.fail()) {
                        ...
                        break;
                    } else CloseSlot((unsigned char)(SID - 1), true);
                }
            }
            ...
            // !CLOSEALL
            //
            else if (Command == "closeall" && !m_GameLoading && !m_GameLoaded && Level >= 9) CloseAllSlots();
            ...
            //
                ...
                if (SS.fail()) CONSOLE_Print("[GAME: " + m_GameName + "] bad input #1 to comp command");
                else {
                    if (!SS.eof()) SS >> Skill;

                    if (SS.fail()) CONSOLE_Print("[GAME: " + m_GameName + "] bad input #2 to comp command");
                    else ComputerSlot((unsigned char)(Slot - 1), (unsigned char) Skill, true);
                }
            }
            ...
            //
                ...
                if (SS.fail()) CONSOLE_Print("[GAME: " + m_GameName + "] bad input #1 to compcolour command");
                else {
                    if (SS.eof()) CONSOLE_Print("[GAME: " + m_GameName + "] missing input #2 to compcolour command");
                    else {
                        SS >> Colour;

                        if (SS.fail()) CONSOLE_Print("[GAME: " + m_GameName + "] bad input #2 to compcolour command");
                        else {
                            unsigned char SID = (unsigned char)(Slot - 1);

                            if (!(m_Map - > GetMapOptions() & MAPOPT_FIXEDPLAYERSETTINGS) && Colour < 12 && SID < m_Slots.size()) {
                                if (m_Slots[SID].GetSlotStatus() == SLOTSTATUS_OCCUPIED && m_Slots[SID].GetComputer() == 1) ColourSlot(SID, Colour);
                            }
                        }
                    }
                }
            ...
            //
                ...
                if (SS.fail()) CONSOLE_Print("[GAME: " + m_GameName + "] bad input #1 to comphandicap command");
                else {
                    if (SS.eof()) CONSOLE_Print("[GAME: " + m_GameName + "] missing input #2 to comphandicap command");
                    else {
                        SS >> Handicap;

                        if (SS.fail()) CONSOLE_Print("[GAME: " + m_GameName + "] bad input #2 to comphandicap command");
                        else {
                            unsigned char SID = (unsigned char)(Slot - 1);
            ...
            //
                ...
                if (SS.fail()) CONSOLE_Print("[GAME: " + m_GameName + "] bad input #1 to comprace command");
                else {
                    if (SS.eof()) CONSOLE_Print("[GAME: " + m_GameName + "] missing input #2 to comprace command");
                    else {
                        getline(SS, Race);
                        string::size_type Start = Race.find_first_not_of(" ");

                        if (Start != string::npos) Race = Race.substr(Start);
                        ...
                        if (!(m_Map - > GetMapOptions() & MAPOPT_FIXEDPLAYERSETTINGS) && !(m_Map - > GetMapFlags() & MAPFLAG_RANDOMRACES) && SID < m_Slots.size()) {
                            if (m_Slots[SID].GetSlotStatus() == SLOTSTATUS_OCCUPIED && m_Slots[SID].GetComputer() == 1) {
                                if (Race == "human") {
                                    ...
                                    SendAllSlotInfo();
                                } else CONSOLE_Print("[GAME: " + m_GameName + "] unknown race [" + Race + "] sent to comprace command");
                            }
                        }
                    }
                }
            ...
            //
                ...
                if (SS.fail()) CONSOLE_Print("[GAME: " + m_GameName + "] bad input #1 to compteam command");
                else {
                    if (SS.eof()) CONSOLE_Print("[GAME: " + m_GameName + "] missing input #2 to compteam command");
                    else {
                        SS >> Team;

                        if (SS.fail()) CONSOLE_Print("[GAME: " + m_GameName + "] bad input #2 to compteam command");
                        else {
                            unsigned char SID = (unsigned char)(Slot - 1);
            ...
            // !DBSTATUS
            //
            else if (Command == "dbstatus" && Level >= 9) SendAllChat(m_GHost - > m_DB - > GetStatus());
            ...
            //
                ...
                if (Matches == 0) SendAllChat(m_GHost - > m_Language - > UnableToStartDownloadNoMatchesFound(Payload));
                else if (Matches == 1) {
                ...
                } else SendAllChat(m_GHost - > m_Language - > UnableToStartDownloadFoundMoreThanOneMatch(Payload));
            }

            //
            // !DROP
            //
            else if (Command == "drop" && m_GameLoaded && Level >= 6) StopLaggers("lagged out (dropped by admin)");
            ...
            //
                ...
                if (m_FakePlayerPID == 255) CreateFakePlayer();
                else DeleteFakePlayer();
            }
            ...
            //
                ...
                if (!Froms.empty()) SendAllChat(Froms);
            }
            ...
            //
                ...
                if (!Payload.empty()) {
                    ...
                    } else SendAllChat(m_GHost - > m_Language - > UnableToSetHCLTooLong());
                } else SendAllChat(m_GHost - > m_Language - > TheHCLIs(m_HCLCommandString));
            }
            ...
            //
                ...
                if (Matches == 0) SendAllChat(m_GHost - > m_Language - > UnableToKickNoMatchesFound(Payload));
                else if (Matches == 1) {
                    uint32_t VictimLevel = 0;
                    string VictimLevelName;
                    for (vector < CBNET * > ::iterator i = m_GHost - > m_BNETs.begin(); i != m_GHost - > m_BNETs.end(); ++i) {
                    ...
                    if (VictimLevel <= 1 || Level >= 9) {
                        ...
                        if (!m_GameLoading && !m_GameLoaded) LastMatch - > SetLeftCode(PLAYERLEAVE_LOBBY);
                        else LastMatch - > SetLeftCode(PLAYERLEAVE_LOST);

                        if (!m_GameLoading && !m_GameLoaded) {
                            OpenSlot(GetSIDFromPID(LastMatch - > GetPID()), false);
                            m_Balanced = false;
                        }
                    } else SendChat(player, "You have no permission to kick this player");
                } else SendAllChat(m_GHost - > m_Language - > UnableToKickFoundMoreThanOneMatch(Payload));
            }
            ...
            //
                ...
                if (Payload.empty()) SendAllChat(m_GHost - > m_Language - > LatencyIs(UTIL_ToString(m_Latency)));
                else {
                    ...
                    } else SendAllChat(m_GHost - > m_Language - > SettingLatencyTo(UTIL_ToString(m_Latency)));
                }
            }
            ...
            //
                ...
                if (Matches == 0) SendAllChat(m_GHost - > m_Language - > UnableToMuteNoMatchesFound(Payload));
                else if (Matches == 1) {
                    uint32_t VictimLevel = 0;
                    string VictimLevelName;
                    for (vector < CBNET * > ::iterator i = m_GHost - > m_BNETs.begin(); i != m_GHost - > m_BNETs.end(); ++i) {
                    ...
                    } else SendChat(player, "You have no permission to mute this player");
                } else SendAllChat(m_GHost - > m_Language - > UnableToMuteFoundMoreThanOneMatch(Payload));
            }
            ...
            // !OPENALL
            //
            else if (Command == "openall" && !m_GameLoading && !m_GameLoaded && Level >= 8) OpenAllSlots();
            ...
            //
                ...
                } else SendAllChat(m_GHost - > m_Language - > UnableToSetGameOwner(m_OwnerName));
            }
            ...
            //
                ...
                if (!m_GameLoading && !m_GameLoaded && !Payload.empty()) KickPing = UTIL_ToUInt32(Payload);
                ...
                for (vector < CGamePlayer * > ::iterator i = SortedPlayers.begin(); i != SortedPlayers.end(); ++i) {
                    ...
                    if (i != SortedPlayers.end() - 1) Pings += ", ";
                ...
                if (!Pings.empty()) SendAllChat(Pings);

                if (Kicked > 0) SendAllChat(m_GHost - > m_Language - > KickingPlayersWithPingsGreaterThan(UTIL_ToString(Kicked), UTIL_ToString(KickPing)));
            }
            ...
            //
                ...
                if (Payload.length() < 31) {
                    ...
                    for (vector < CBNET * > ::iterator i = m_GHost - > m_BNETs.begin(); i != m_GHost - > m_BNETs.end(); ++i) {
                        ...
                        if (( * i) - > GetPasswordHashType() != "pvpgn")( * i) - > QueueEnterChat();
                    }

                    m_CreationTime = GetTime();
                    m_LastRefreshTime = GetTime();
                } else SendAllChat(m_GHost - > m_Language - > UnableToCreateGameNameTooLong(Payload));
            }
            ...
            //
                ...
                for (vector < CBNET * > ::iterator i = m_GHost - > m_BNETs.begin(); i != m_GHost - > m_BNETs.end(); ++i)( * i) - > QueueChatCommand(Payload);
            ...
            //
                ...
                if (SS.fail()) CONSOLE_Print("[GAME: " + m_GameName + "] bad inputs to sendlan command");
                else {
            ...
            //
                ...
                if (Payload == "force") StartCountDown(true);
                else {
                    if (GetTicks() - m_LastPlayerLeaveTicks >= 2000) StartCountDown(false);
                    else SendAllChat(m_GHost - > m_Language - > CountDownAbortedSomeoneLeftRecently());
                }
            }
            ...
            //
                ...
                if (SS.fail()) CONSOLE_Print("[GAME: " + m_GameName + "] bad input #1 to swap command");
                else {
                    if (SS.eof()) CONSOLE_Print("[GAME: " + m_GameName + "] missing input #2 to swap command");
                    else {
                        SS >> SID2;

                        if (SS.fail()) CONSOLE_Print("[GAME: " + m_GameName + "] bad input #2 to swap command");
                        else {
            ...
            //
                ...
                if (Payload.empty()) SendAllChat(m_GHost - > m_Language - > SyncLimitIs(UTIL_ToString(m_SyncLimit)));
                else {
                    ...
                    } else SendAllChat(m_GHost - > m_Language - > SettingSyncLimitTo(UTIL_ToString(m_SyncLimit)));
                }
            }
            ...
            // !UNHOST
            //
            else if (Command == "unhost" && !m_CountDownStarted && Level >= 8) m_Exiting = true;
            ...
            //
                ...
                if (MessageStart != string::npos) {
                    ...
                    for (vector < CBNET * > ::iterator i = m_GHost - > m_BNETs.begin(); i != m_GHost - > m_BNETs.end(); ++i)( * i) - > QueueChatCommand(Message, Name, true);
                }
        ...
        if (!player - > GetSpoofed()) CONSOLE_Print("[GAME: " + m_GameName + "] non-spoofchecked user [" + User + "] sent command [" + Command + "] with payload [" + Payload + "]");
        else CONSOLE_Print("[GAME: " + m_GameName + "] non-admin [" + User + "] sent command [" + Command + "] with payload [" + Payload + "]");
    }
    ...
    if (Command == "checkme") {
        ...
        if (player - > GetKickVote()) SendChat(player, "[INFO] You already voted to forfeit the game. See current results with '!wff'");
        if (player - > GetDrawVote()) SendChat(player, "[INFO] You already voted to draw the game. You can change your mind by using '!undraw'");
        string IgnoredPlayers;
        for (vector < string > ::iterator i = player - > m_IgnoreList.begin(); i != player - > m_IgnoreList.end(); ++i) {
            if (IgnoredPlayers.empty()) IgnoredPlayers = * i;
            else IgnoredPlayers = ", " + * i;
        }
        if (!IgnoredPlayers.empty()) SendChat(player, "[INFO] Ignored players: " + IgnoredPlayers);
        if (player - > GetAFKMarked()) SendChat(player, "[WARNING] You got already marked as an AFKer, if you are AFK once again for more than 3 mins you will be kicked with an autoban.");
        if (player - > GetFeedLevel() == 1) SendChat(player, "[WARNING] You are already marked as a feeder, might wanna be careful about dying.");
        if (player - > GetFeedLevel() == 2) SendChat(player, "[WARNING] You are marked as a complete feeder, you could be votekicked.");
        if (player - > GetHighPingTimes() > 0) SendChat(player, "[WARNING] You have been marked already [" + UTIL_ToString(player - > GetHighPingTimes()) + "] as a high-ping player.");
    }
    //
    // !STATS
    //
    ...
    if (Command == "stats" && GetTime() - player - > GetStatsSentTime() >= 5) {
        ...
        if (Matches == 0) {
            if (player - > GetSpoofed() && Level >= 8) m_PairedGSChecks.push_back(PairedGSCheck(string(), m_GHost - > m_DB - > ThreadedStatsPlayerSummaryCheck(StatsUser)));
            else m_PairedGSChecks.push_back(PairedGSCheck(User, m_GHost - > m_DB - > ThreadedStatsPlayerSummaryCheck(StatsUser)));
        } else if (Matches == 1) {
            if (player - > GetSpoofed() && Level >= 8) m_PairedGSChecks.push_back(PairedGSCheck(string(), m_GHost - > m_DB - > ThreadedStatsPlayerSummaryCheck(LastMatch - > GetName())));
            else m_PairedGSChecks.push_back(PairedGSCheck(User, m_GHost - > m_DB - > ThreadedStatsPlayerSummaryCheck(LastMatch - > GetName())));
        } else if (Matches > 1) SendChat(player, "Error, found to many name partial matching on [" + StatsUser + "]");
    ...
    if ((Command == "rank" || Command == "class") && GetTime() - player - > GetStatsSentTime() >= 5) {
        ...
        uint32_t Matches = GetPlayerFromNamePartial(StatsUser, & LastMatch);
        if (Matches == 0) {
            if (player - > GetSpoofed() && Level >= 8) m_PairedRankChecks.push_back(PairedRankCheck(string(), m_GHost - > m_DB - > ThreadedStatsPlayerSummaryCheck(StatsUser)));
            else m_PairedRankChecks.push_back(PairedRankCheck(User, m_GHost - > m_DB - > ThreadedStatsPlayerSummaryCheck(StatsUser)));
        } else if (Matches == 1) {
            if (player - > GetSpoofed() && Level >= 8) m_PairedRankChecks.push_back(PairedRankCheck(string(), m_GHost - > m_DB - > ThreadedStatsPlayerSummaryCheck(LastMatch - > GetName())));
            else m_PairedRankChecks.push_back(PairedRankCheck(User, m_GHost - > m_DB - > ThreadedStatsPlayerSummaryCheck(LastMatch - > GetName())));
        } else if (Matches > 1) SendChat(player, "Error, found to many name partial matching on [" + StatsUser + "]");
    ...
    //
        ...
        uint32_t Matches = GetPlayerFromNamePartial(StatsUser, & LastMatch);
        if (Matches == 0) {
            if (player - > GetSpoofed() && Level >= 8) m_PairedSChecks.push_back(PairedSCheck(string(), m_GHost - > m_DB - > ThreadedStatsPlayerSummaryCheck(StatsUser)));
            else m_PairedSChecks.push_back(PairedSCheck(User, m_GHost - > m_DB - > ThreadedStatsPlayerSummaryCheck(StatsUser)));
        } else if (Matches == 1) {
            if (player - > GetSpoofed() && Level >= 8) m_PairedSChecks.push_back(PairedSCheck(string(), m_GHost - > m_DB - > ThreadedStatsPlayerSummaryCheck(LastMatch - > GetName())));
            else m_PairedSChecks.push_back(PairedSCheck(User, m_GHost - > m_DB - > ThreadedStatsPlayerSummaryCheck(LastMatch - > GetName())));
        } else if (Matches > 1) SendChat(player, "Error, found to many name partial matching on [" + StatsUser + "]");
    ...
    //
        ...
        if (player - > GetSpoofed() && Level >= 8) SendAllChat(m_GHost - > m_Language - > VersionAdmin(m_GHost - > m_Version));
        else SendAllChat(m_GHost - > m_Language - > VersionNotAdmin(m_GHost - > m_Version));
    }
    ...
    //
        ...
        for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
            if (( * i) - > GetKickVote()) {
                ++Votes;
                if (!VotedPlayers.empty()) VotedPlayers += ", " + ( * i) - > GetName();
                else VotedPlayers = ( * i) - > GetName();
            }
        }
    ...
    //
        ...
        player - > SetVKTimes();
        if (player - > GetVKTimes() == 8) m_PairedBanAdds.push_back(PairedBanAdd(User, m_GHost - > m_DB - > ThreadedBanAdd(player - > GetJoinedRealm(), player - > GetName(), player - > GetExternalIPString(), m_GameName, "PeaceMaker", "votekick abuse", 432000, "")));
        else if (player - > GetVKTimes() == 5) m_Pairedpenps.push_back(Pairedpenp(string(), m_GHost - > m_DB - > Threadedpenp(player - > GetName(), "votekick abuse", "PeaceMaker", 1, "add")));
        else if (player - > GetVKTimes() >= 2) SendChat(player, "[INFO] Abusive usage of the votekick command is banable.");

        if (!m_KickVotePlayer.empty()) SendChat(player, m_GHost - > m_Language - > UnableToVoteKickAlreadyInProgress());
        else if (m_Players.size() <= 3) SendChat(player, m_GHost - > m_Language - > UnableToVoteKickNotEnoughPlayers());
        else {
            string name;
            string reason;
            stringstream SS;
            ...
            if (!CustomVoteKickReason(reason)) {
                ...
                if (Matches == 0) SendChat(player, m_GHost - > m_Language - > UnableToVoteKickNoMatchesFound(name));
                else if (Matches == 1) {
                    ...
                    if (VLevel > 2 && Level < VLevel) SendChat(player, "[INFO] You may not votekick this player.");
                    else if (OnlyPlayer) SendChat(player, "[INFO] Unable to votekick player [" + LastMatch - > GetName() + "] cannot votekick when there is only one player on the victim's team.");
                    else if (LastMatch == player) SendChat(player, "[INFO] You cannot votekick yourself!");
                    else {
                        m_KickVotePlayer = LastMatch - > GetName();
                        m_StartedKickVoteTime = GetTime();

                        for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i)( * i) - > SetKickVote(false);
                ...
                } else SendChat(player, m_GHost - > m_Language - > UnableToVoteKickFoundMoreThanOneMatch(name));
            }
        }
    }
    ...
    //
        ...
        for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
            if (( * i) - > GetName() == m_KickVotePlayer) voteplayerteam = m_Slots[GetSIDFromPID(( * i) - > GetPID())].GetTeam();
        }
        if (m_Slots[GetSIDFromPID(player - > GetPID())].GetTeam() == voteplayerteam) {
            for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
                if (( * i) - > GetKickVote())++Votes;
            }
            ...
            if (Votes >= VotesNeeded) {
                ...
                if (Victim) {
                    ...
                    if (!m_GameLoading && !m_GameLoaded) Victim - > SetLeftCode(PLAYERLEAVE_LOBBY);
                    else Victim - > SetLeftCode(PLAYERLEAVE_LOST);

                    if (!m_GameLoading && !m_GameLoaded) OpenSlot(GetSIDFromPID(Victim - > GetPID()), false);
                ...
                } else SendAllChat(m_GHost - > m_Language - > ErrorVoteKickingPlayer(m_KickVotePlayer));

                m_KickVotePlayer.clear();
                m_StartedKickVoteTime = 0;
            } else SendAllChat(m_GHost - > m_Language - > VoteKickAcceptedNeedMoreVotes(m_KickVotePlayer, User, UTIL_ToString(VotesNeeded - Votes)));
        } else SendChat(player, "You may only vote for kicking players from your team.");
    ...
    if (m_GameLoaded && (Command == "draw" || Command == "undraw") && !m_SoftGameOver) {
        if (Command == "draw") {
            ...
            if (!player - > GetDrawVote()) player - > SetDrawVote(true);
            else ChangedVote = false; //continue in case someone left and now we have enough votes
    ...
    if (m_GameLoaded && m_ForfeitTime == 0 && (Command == "ff" || Command == "forfeit") && !m_SoftGameOver) {
        if (GetTime() - m_GameLoadedTime <= (m_GHost - > m_MinFF * 60 - 200 * m_Leavers)) SendChat(player, "[INFO] You may FF after [20] minutes, [" + UTIL_ToString(m_GHost - > m_MinFF - m_Leavers * 2 - (GetTime() - m_GameLoadedTime)) + "] minutes remaining.");
        else {
            bool ChangedVote = true;

            if (!player - > GetForfeitVote()) player - > SetForfeitVote(true);
            else ChangedVote = false;
                ...
                for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); i++) {
                    if ( * i && !( * i) - > GetLeftMessageSent()) {
                        ...
                        if (sid != 255 && m_Slots[sid].GetTeam() == playerTeam) {
                            ...
                            if (!( * i) - > GetForfeitVote()) AllVoted = false;
                            else numVoted++;
                        }
                    }
                }
    ...
    //
        ...
        for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); i++) {
            ...
            if (sid <= 4 && sid >= 0) {
                if (( * i) - > GetForfeitVote()) {
                    if (!SeNames.empty()) SeNames += ", " + ( * i) - > GetName();
                    else SeNames = ( * i) - > GetName();
                    SeVotes++;
                }
                SePlayers++;
            } else if (sid >= 5 && sid <= 9) {
                if (( * i) - > GetForfeitVote()) {
                    ...
                    if (!ScNames.empty()) ScNames += ", " + ( * i) - > GetName();
                    else ScNames = ( * i) - > GetName();
                    ScVotes++;
                }
                ScPlayers++;
            } else if (m_Slots[sid].GetTeam() != 12) twoteammap = false;
            Votes++;
        }
        if (Votes == 0) SendChat(player, "No one has voted for forfeiting the game yet");
        else if (!twoteammap) SendChat(player, "Error, this is not a two-team map, you cannot forfeit here");
        else {
    ...
    //
        ...
        if (Level > 2) {
            ...
            } else SendChat(player, "Error. There is already a game observer.");
        } else SendChat(player, "Error. You require at least to be a safelisted member to have access to this command.");
    }
    ...
    //
        ...
        if (Level > 2 && SID == 11) {
            ...
            if (newslot == -1) newslot = m_LatestSlot;
    ...
    //
        ...
        if (!SS.eof()) {
            ...
            if (Start != string::npos) Status = Status.substr(Start);
        }
        ...
        } else if (Status == "0" || Status == "clear") m_PairedPassChecks.push_back(PairedPassCheck(User, m_GHost - > m_DB - > ThreadedPassCheck(User, Password, 1)));
        else SendChat(player, "Error wrong status, please use 'clear' or '0' to remove the password protection");
    ...
    // !I   !INBOX
    //
    else if ((Command == "i" || Command == "inbox") && Payload.empty()) m_PairedINChecks.push_back(PairedINCheck(User, m_GHost - > m_DB - > ThreadedInboxSummaryCheck(User)));
    ...
    //
        ...
        if (!SS.eof()) {
            ...
            if (Start != string::npos) Message = Message.substr(Start);
        } else SendChat(player, "Error. Wrong input, please use '!pm <user> <message>'");

        if (UserTo.length() >= 3) {
            if (Payload.length() > 3) {
                m_Pairedpms.push_back(Pairedpm(User, m_GHost - > m_DB - > Threadedpm(User, UserTo, 0, Message, "add")));
            } else SendChat(player, "Error. The message is to short.");
        } else SendChat(player, "Error. This User is invalid");
    }
    ...
    //
        ...
        if (!StatsUser.empty() && StatsUser.size() < 16 && StatsUser[0] != '/') m_PairedStreakChecks.push_back(PairedStreakCheck(User, m_GHost - > m_DB - > ThreadedStatsPlayerSummaryCheck(StatsUser)));
    }
    ...
    //
        ...
        if (!StatsUser.empty() && StatsUser.size() < 16 && StatsUser[0] != '/') m_PairedSSs.push_back(PairedSS(User, m_GHost - > m_DB - > ThreadedStatsSystem(StatsUser, "betsystem", 0, "betcheck")));
    }
    ...
    //
        ...
        if (GetTime() - m_GameLoadedTime >= 300) {
            ...
            return HideCommand;
        } else m_PairedSSs.push_back(PairedSS(User, m_GHost - > m_DB - > ThreadedStatsSystem(User, "betsystem", UTIL_ToUInt32(Payload), "bet")));
    }
    ...
    //
        ...
        if (Level > 2) {
            if (!m_PauseReq) {
                ...
                m_Paused = false;
            } else SendChat(player, "Error, someone already requested to pause the game.");
        } else SendChat(player, "Error, you need to be at least a safelisted player to request a pause.");
    }
    ...
    //
        ...
        if (Level > 2) {
            if (m_Paused) {
                ...
                m_PauseTicks = 5;
                m_LastCountDownTicks = 0;
            } else SendChat(player, "Error, the game isn't pause.");
        } else SendChat(player, "Error, you need to be at least a safelisted player to request a pause.");
    }
    ...
    //
        ...
        for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
            ...
            if (sid <= 4 && sid >= 0) {
                int Win = 0;
                if (( * i) - > GetGames() >= 10) Win = (( * i) - > GetWinPerc() * ( * i) - > GetGames());
                else if (Win == 0) Win = (33 * ( * i) - > GetGames());
                else if (( * i) - > GetGames() == 0) Win = 33;
                ...
                m_TotalWinPoints += Win;
            } else if (m_Slots[sid].GetTeam() != 12) twoteam = false;
        ...
        if (twoteam) {
            ...
            } else SendChat(player, "Error. Currently one team hasn't got a player recorded");
        } else SendChat(player, "Error, this isn't a two-team map");
    }
    ...
    //
        ...
        if (Matches != 1) return HideCommand;
        ...
        if (LastMatch - > GetName() != User) {
            if (RandomNumber == 0) SendAllChat(User + " slaps " + LastMatch - > GetName() + " with a large trout.");
            else if (RandomNumber == 1) SendAllChat(User + " slaps " + LastMatch - > GetName() + " with a pink Macintosh.");
            else if (RandomNumber == 2) SendAllChat(User + " throws a Playstation 3 at " + LastMatch - > GetName() + ".");
            else if (RandomNumber == 3) SendAllChat(User + " drives a car over " + LastMatch - > GetName() + ".");
                ...
                } else SendAllChat("But " + LastMatch - > GetName() + " hadn't any cookies :( ");
            } else if (RandomNumber == 5) SendAllChat(User + " washes " + LastMatch - > GetName() + "'s car.  Oh, the irony!");
            else if (RandomNumber == 6) SendAllChat(User + " burns " + LastMatch - > GetName() + "'s house.");
            else if (RandomNumber == 7) SendAllChat(User + " finds " + LastMatch - > GetName() + "'s picture on uglypeople.com.");
        } else {
            if (RandomNumber == 0) SendAllChat(User + " slaps himself with a large trout.");
            else if (RandomNumber == 1) SendAllChat(User + " slaps himself with a pink Macintosh.");
            else if (RandomNumber == 2) SendAllChat(User + " throws a Playstation 3 at himself.");
            else if (RandomNumber == 3) SendAllChat(User + " drives a car over himself.");
            else if (RandomNumber == 4) SendAllChat(User + " steals his cookies. mwahahah!");
            else if (RandomNumber == 5) SendAllChat(User + " searches yahoo.com for goatsex + " + User + ". " + UTIL_ToString(rand()) + " hits WEEE!");
            else if (RandomNumber == 6) SendAllChat(User + " burns his house.");
            else if (RandomNumber == 7) SendAllChat(User + " finds his picture on uglypeople.com.");
        }
    }
    ...
    //
        ...
        } else SendChat(player, "Error, you dont have a cookie!");
    }
    ...
    //
        ...
        if (Matches == 0) SendChat(player, "Unable to ignore player [" + Payload + "]. No matches found.");
        else if (Matches == 1) {
        ...
        } else SendChat(player, "Unable to ignore player [" + Payload + "]. Found more than one match.");
    }
    ...
    //
        ...
        if (Matches == 0) SendChat(player, "Unable to unignore player [" + Payload + "]. No matches found.");
        else if (Matches == 1) {
            player - > UnIgnore(LastMatch - > GetName());
            SendChat(player, "You have unignored player [" + LastMatch - > GetName() + "].");
        } else SendChat(player, "Unable to unignore player [" + Payload + "]. Found more than one match.");
    }
    ...
    //
        ...
        if (Payload == "leave" || Payload == "leaving") SendChat(player, "[Leaving] Leave the game more than 3 min. before tree/throne is dead.");
        else if (Payload == "flame" || Payload == "flameing") SendChat(player, "[Flaming] Insult other people, using words like 'noob', 'fuck you', 'bastard', 'motherfucker', and so on. It is admins decision which words count as flaming!.");
        else if (Payload == "afk") SendChat(player, "[AFK] Being afk and/or not playing any more. Also moving around at fountain / in base. It is admins decision what counts as afk!.");
        else if (Payload == "spam" || Payload == "spaming") SendChat(player, "[Spam] Chatting very much text in very less time will be muted and banned.");
        else if (Payload == "fountainfarm") SendChat(player, "[FountainFarm] Kill enemies inside the fountain area (healing area). Does not invlude Zeus ulti. At the end, its admins decision what counts as fountain farm.");
        ...
        } else if (Payload == "hack" || Payload == "hacking") SendChat(player, "[Hack] Using tools like maphack (or any other hacking tools) or abusing known bugs of the map. At the end, its admins decision what counts as bug abusing.");
        else {
...
bool CGame::CustomVoteKickReason(string reason) {
    ...
    if (reason.find("maphack") != string::npos || reason.find("fountainfarm") != string::npos || reason.find("feeding") != string::npos || reason.find("flaming") != string::npos || reason.find("gameruin") != string::npos) return true;
