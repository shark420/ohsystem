		...
		if( ( m_Locked && Level > 8 ) || !m_Locked )
		{
			...
			m_AdminLog.push_back( User + " gl" + "\t" + UTIL_ToString( Level ) + "\t" + Command + "\t" + Payload );
				...
				else if(Command=="simulatechat"&&!Payload.empty() )
				{
 					...
 					else
					{
  						...
  						else
						{
							...
							{
    								SendChat( player,"Successfully send a simulated message");
                                        ...
                                        else
                                        {
                                                ...
                                                else
                                                {
					                                ...
					                                m_Pairedpenps.push_back( Pairedpenp( string(), m_GHost->m_DB->Threadedpenp( LastMatch->GetName( ), Reason, User, UTIL_ToUInt32( Amount ), "add" ) ) );
                                                                ...
                                                                SendChat( player, "Error. Please add a reason to punish someone" );
                                                }
                                        }
                                }
				...
				else if( Command == "setcookies" && Level >= 9 )
				{
					if( Payload.empty( ) )
					{
						SendAllChat( "Player "+player->GetName()+" refilled his cookiechar." );
						player->SetCookie( 3 );
					}
					else
					{
			...
			******************/
			...
			// !ABC
			//
			else if ( ( Command == "autobalance" || Command == "ab" ) && Level == 10 && Payload.empty() )
			{
		                OHFixedBalance( );
			}
			...
			else if( Command == "announce" && !m_CountDownStarted && Level >= 6 )
			{
	...
	*********************/
	...
	if( Command == "checkme" )
	{
		...
		if( player->GetAFKMarked( ) )
			SendChat( player, "[WARNING] You got already marked as AFK, if you are another time more than 3 minutes AFK you will be kicked with an autoban." );
		if( player->GetFeedLevel( ) == 1 )
			 SendChat( player, "[WARNING] You are already marked as feeder, may decrease your amount of dieing." );
		if( player->GetFeedLevel( ) == 2 )
			SendChat( player, "[WARNING] You are marked completely as feeder, you could be votekicked." );
		if( player->GetHighPingTimes( ) > 0 )
			SendChat( player, "[WARNING] You have been marked already ["+UTIL_ToString( player->GetHighPingTimes( ) )+"] for a high ping." );
	}
	//
	// !STATS
	//
	...
	else if( Command == "votekick" && m_GHost->m_VoteKickAllowed && !Payload.empty( ) )
	{
		...
		if( player->GetVKTimes( ) == 8 )
			...
			SendChat( player, "[INFO] Abusive usage of the votekick command is banable. Please visit rules on chevelle1.net." );
		...
		else
		{
			...
			else
			{
				...
				else if( Matches == 1 )
				{
					...
					{
						SendChat( player, "[INFO] Player ["+LastMatch->GetName( )+"] wasnt marked as feeder. Adding one additional abuse point for votekick-command." );
						SendChat( player, "[INFO] Abuse points will be collected and if you have to many this game, you will be banned." );
						player->SetVKTimes( );
						return HideCommand;
					}
					...
					if( VLevel > 2 && Level < VLevel )
						...
						SendChat( player, "[INFO] Unable to votekick player [" + LastMatch->GetName( ) + "] cannot votekick when there is only one player on victim's team." );
			...
			if( playerSID != 255 )
			{
				...
				if( m_ForfeitTeam == 0 || m_ForfeitTeam == 1 )
				{
					...
					if( AllVoted )
					{
						...
						SendAllChat( "Please stay till the gameover timer has finished, to keep your stats!" );
					}
	...
	//
        ...
        else if( Command == "pm" && !Payload.empty( ) )
        {
        	string UserTo;
                ...
                if( UserTo.length() >= 3 )
                {
        	        ...
        	        SendChat( player, "Error. This User is not valied" );
        }
		...
		if( !Payload.find_first_not_of( "1234567890" ) == string :: npos )
                {
	        	SendChat( player, "Error. You should bet a number :D" );
			return HideCommand;
                }
		if( UTIL_ToUInt32( Payload ) > 50 || UTIL_ToUInt32( Payload ) <= 0 )
		{
			SendChat( player, "Error. You shouldn't bet a amount over 50 and not lower than 0 or 0." );
			return HideCommand;
		}
		...
		if( GetTime() - m_GameLoadedTime >= 300 )
	        {
                        SendChat( player, "Error. You may should not bet yet, its already to late" );
                        return HideCommand;
                }
		else
	...
	//
	else if( ( Command == "wp" || Command == "cb" || Command == "winperc" || Command == "ceckbalance" ) && Payload.empty( ) )
	{
		...
		if( twoteam )
		{
			if( m_TotalWinPoints != 0 )
			{
				...
				SendChat( player, "Error. Currently one team hasn't a player" );
		}
		else
