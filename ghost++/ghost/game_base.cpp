...
CBaseGame::CBaseGame(CGHost * nGHost, CMap * nMap, CSaveGame * nSaveGame, uint16_t nHostPort, unsigned char nGameState, string nGameName, string nOwnerName, string nCreatorName, string nCreatorServer, uint32_t nGameType): m_GHost(nGHost),
...
m_PauseReq(false),
m_PauseTicks(5),
m_SendPauseInfo(false),
m_GameNoGarena(false),
m_LastInGameAnnounce(0) {
    ...
    if (m_GHost - > m_SaveReplays && !m_SaveGame) m_Replay = new CReplay();
    ...
    if (m_GHost - > m_ReconnectWaitTime != 0) {
        ...
        if (m_GProxyEmptyActions > 9) m_GProxyEmptyActions = 9;
    }
    ...
    } else m_Slots = m_Map - > GetSlots();

    if (!m_GHost - > m_IPBlackListFile.empty()) {
        ifstream in ; in .open(m_GHost - > m_IPBlackListFile.c_str());

        if ( in .fail()) CONSOLE_Print("[GAME: " + m_GameName + "] error loading IP blacklist file [" + m_GHost - > m_IPBlackListFile + "]");
        else {
            ...
            while (! in .eof()) {
                ...
                if (Line.find_first_not_of("1234567890.") != string::npos) continue;
    ...
    if (!m_GHost - > m_BindAddress.empty()) CONSOLE_Print("[GAME: " + m_GameName + "] attempting to bind to address [" + m_GHost - > m_BindAddress + "]");
    else CONSOLE_Print("[GAME: " + m_GameName + "] attempting to bind to all available addresses");

    if (m_Socket - > Listen(m_GHost - > m_BindAddress, m_HostPort)) CONSOLE_Print("[GAME: " + m_GameName + "] listening on port " + UTIL_ToString(m_HostPort));
    else {
...
CBaseGame::~CBaseGame() {
    ...
    if (m_Replay && (m_GameLoading || m_GameLoaded)) {
        ...
        if (MinString.size() == 1) MinString.insert(0, "0");

        if (SecString.size() == 1) SecString.insert(0, "0");
        ...
        if (m_GHost - > m_GameIDReplays) {
            // By uakf.b
            if (m_DatabaseID == 0) m_Replay - > Save(m_GHost - > m_TFT, m_GHost - > m_ReplayPath + UTIL_FileSafeName("GHost++ " + string(Time) + " " + m_GameName + " (" + MinString + "m" + SecString + "s).w3g"));
            else m_Replay - > Save(m_GHost - > m_TFT, m_GHost - > m_ReplayPath + UTIL_FileSafeName("GHost++ " + UTIL_ToString(m_DatabaseID) + ".w3g"));
        } else {
    ...
    if (m_CallablePList) m_GHost - > m_Callables.push_back(m_CallablePList);
...
uint32_t CBaseGame::GetNextTimedActionTicks() {
    ...
    if (!m_GameLoaded || m_Lagging) return 50;

    uint32_t TicksSinceLastUpdate = GetTicks() - m_LastActionSentTicks;

    if (TicksSinceLastUpdate > m_Latency - m_LastActionLateBy) return 0;
    else return m_Latency - m_LastActionLateBy - TicksSinceLastUpdate;
}
...
uint32_t CBaseGame::GetSlotsOccupied() {
    ...
    for (vector < CGameSlot > ::iterator i = m_Slots.begin(); i != m_Slots.end(); ++i) {
        if (( * i).GetSlotStatus() == SLOTSTATUS_OCCUPIED)++NumSlotsOccupied;
    }
...
uint32_t CBaseGame::GetSlotsAllocated() {
    ...
    if (NumPlayers > SlotsOccupied) return NumPlayers;
    else return SlotsOccupied;
}
...
uint32_t CBaseGame::GetSlotsOpen() {
    ...
    for (vector < CGameSlot > ::iterator i = m_Slots.begin(); i != m_Slots.end(); ++i) {
        if (( * i).GetSlotStatus() == SLOTSTATUS_OPEN)++NumSlotsOpen;
    }
...
uint32_t CBaseGame::GetNumPlayers() {
    ...
    if (m_FakePlayerPID != 255)++NumPlayers;
...
uint32_t CBaseGame::GetNumHumanPlayers() {
    ...
    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
        if (!( * i) - > GetLeftMessageSent())++NumHumanPlayers;
    }
...
string CBaseGame::GetDescription() {
    ...
    if (m_GameLoading || m_GameLoaded) Description += " : " + UTIL_ToString((m_GameTicks / 1000) / 60) + "m";
    else Description += " : " + UTIL_ToString((GetTime() - m_CreationTime) / 60) + "m";
...
bool CBaseGame::Update(void * fd, void * send_fd) {
    ...
    for (vector < CCallableScoreCheck * > ::iterator i = m_ScoreChecks.begin(); i != m_ScoreChecks.end();) {
        if (( * i) - > GetReady()) {
            ...
            for (vector < CPotentialPlayer * > ::iterator j = m_Potentials.begin(); j != m_Potentials.end(); ++j) {
                if (( * j) - > GetJoinPlayer() && ( * j) - > GetJoinPlayer() - > GetName() == ( * i) - > GetName()) EventPlayerJoinedWithScore( * j, ( * j) - > GetJoinPlayer(), Score);
            }
    ...
    for (vector < PairedBanCheck2 > ::iterator i = m_PairedBanCheck2s.begin(); i != m_PairedBanCheck2s.end();) {
        if (i - > second - > GetReady()) {
            ...
            if (UTIL_ToUInt32(Result) >= 5 && i - > second - > GetType() == "joincheck" && m_GHost - > m_CheckIPRange) SendAllChat("Potential banavoider joined: " + i - > second - > GetUser() + " has " + Result + " bans on the IPRange");
    ...
    for (vector < Pairedpenp > ::iterator i = m_Pairedpenps.begin(); i != m_Pairedpenps.end();) {
        if (i - > second - > GetReady()) {
            ...
            if (i - > second - > GetType() == "check") {
                if (Result != 0) SendAllChat("User [" + i - > second - > GetName() + "] has [" + UTIL_ToString(Result) + "] penality point(s).");
                else SendAllChat("User [" + i - > second - > GetName() + "] has no penality points.");
            }
            if (i - > second - > GetType() == "checkall") {
                if (Result != 0) SendAllChat("User [" + i - > second - > GetName() + "] has [" + UTIL_ToString(Result) + "] penality point(s).");
                else SendAllChat("User [" + i - > second - > GetName() + "] has no penality points.");
            } else if (i - > second - > GetType() == "add") {
                if (Result == 1) SendAllChat("Added [" + UTIL_ToString(i - > second - > GetAmount()) + "] penality point(s) for [" + i - > second - > GetName() + "] by [" + i - > second - > GetAdmin() + "]");
                else if (Result == 2) SendAllChat("User [" + i - > second - > GetName() + "] got banned by reaching to many penality points");
                else CONSOLE_Print("Couldn't add a penality point");
            } else CONSOLE_Print("Error. Something gone wrong");
    ...
    for (vector < PairedBanAdd > ::iterator i = m_PairedBanAdds.begin(); i != m_PairedBanAdds.end();) {
        if (i - > second - > GetReady()) {
            ...
            if (Result == "alreadypermbanned") SendAllChat("Error banning user. The User [" + i - > second - > GetUser() + "] is already permanent banned");
            else if (Result == "alreadybannedwithhigheramount") SendAllChat("Error banning user. The User [" + i - > second - > GetUser() + "] is already banned for a longer amount.");
            else if (!Result.empty() && Result != "failed") {
                for (vector < CBNET * > ::iterator j = m_GHost - > m_BNETs.begin(); j != m_GHost - > m_BNETs.end(); ++j) {
                    if (( * j) - > GetServer() == i - > second - > GetServer())( * j) - > AddBan(i - > second - > GetUser(), i - > second - > GetIP(), i - > second - > GetGameName(), i - > second - > GetAdmin(), i - > second - > GetReason());
                }
                SendAllChat(Result);
            } else SendAllChat("Something gone wrong, report this please to Grief-Code.");
    ...
    for (vector < CPotentialPlayer * > ::iterator i = m_Potentials.begin(); i != m_Potentials.end();) {
        if (( * i) - > Update(fd)) {
            ...
            if (( * i) - > GetSocket())( * i) - > GetSocket() - > DoSend((fd_set * ) send_fd);
    ...
    if (!m_GameLoading && !m_GameLoaded && GetSlotsAllocated() < m_Slots.size()) CreateVirtualHost();
    ...
    if (!m_RefreshError && !m_CountDownStarted && m_GameState == GAME_PUBLIC && GetSlotsOpen() > 0 && GetTime() - m_LastRefreshTime >= 3) {
        ...
        if (m_RefreshMessages && Refreshed) SendAllChat(m_GHost - > m_Language - > GameRefreshed());
    ...
    if (!m_GameLoading && !m_GameLoaded && GetTicks() - m_LastDownloadCounterResetTicks >= 1000) {
        ...
        if (m_SlotInfoChanged) SendAllSlotInfo();
    ...
    if (!m_GameLoading && !m_GameLoaded && GetTicks() - m_LastDownloadTicks >= 100) {
        ...
        for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
            if (( * i) - > GetDownloadStarted() && !( * i) - > GetDownloadFinished()) {
                ...
                if (m_GHost - > m_MaxDownloaders > 0 && Downloaders > m_GHost - > m_MaxDownloaders) break;
                ...
                while (( * i) - > GetLastMapPartSent() < ( * i) - > GetLastMapPartAcked() + 1442 * 100 && ( * i) - > GetLastMapPartSent() < MapSize) {
                    ...
                    if (m_GHost - > m_MaxDownloadSpeed > 0 && m_DownloadCounter > m_GHost - > m_MaxDownloadSpeed * 1024) break;
    ...
    if (!m_SendGameLoaded && m_GameLoaded && GetTime() - m_GameLoadedTime >= 300) {
        ifstream in ; in .open(m_GHost - > m_GameLoadedFile.c_str());
        if (! in .fail()) {
            ...
            while (! in .eof() && Count < 8) {
                getline( in , Line);
                if (Line.empty()) {
                    if (! in .eof()) SendAllChat(" ");
                } else SendAllChat(Line);
                ++Count;
            } in .close();
        }
        ...
        for (vector < CGamePlayer * > ::iterator k = m_Players.begin(); k != m_Players.end(); ++k) {
            ...
            if (Level == 0) {
                ...
                for (vector < string > ::iterator i = m_LimitedCountries.begin(); i != m_LimitedCountries.end();) {
                    if ( * i == CC && m_DenieCountries) unallowedcountry = true;

                    if ( * i != CC && m_LimitCountries) unallowedcountry = true;
                    i++;
                }

                for (vector < string > ::iterator i = m_GHost - > m_DCountries.begin(); i != m_GHost - > m_DCountries.end();) {
                    if ( * i == CC) unallowedcountry = true;
                    i++;
                }
    ...
    if (m_CountDownStarted && GetTicks() - m_LastCountDownTicks >= 500 && !m_GameLoaded) {
        if (m_CountDownCounter > 0) {
            ...
            --m_CountDownCounter;
        } else if (!m_GameLoading && !m_GameLoaded) EventGameStarted();
    ...
    if (!m_GameLoading && !m_GameLoaded && m_AutoStartPlayers == 0 && m_GHost - > m_LobbyTimeLimit > 0) {
        ...
        for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
            if (( * i) - > GetReserved()) m_LastReservedSeen = GetTime();
        }
    ...
    if (m_GameLoading) {
        ...
        if (FinishedLoading) {
            ...
            if (m_LoadInGame && GetTime() - m_LastLagScreenResetTime >= 30) {
                ...
                for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
                    if (( * i) - > GetGProxy()) UsingGProxy = true;
                }
                ...
                for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
                    if (( * i) - > GetFinishedLoading()) {
                        ...
                        for (vector < CGamePlayer * > ::iterator j = m_Players.begin(); j != m_Players.end(); ++j) {
                            if (!( * j) - > GetFinishedLoading()) Send( * i, m_Protocol - > SEND_W3GS_STOP_LAG( * j, true));
                        }
                        ...
                        if (UsingGProxy && !( * i) - > GetGProxy()) {
                            ...
                            for (unsigned char j = 0; j < m_GProxyEmptyActions; ++j)( * i) - > AddLoadInGameData(m_Protocol - > SEND_W3GS_INCOMING_ACTION(queue < CIncomingAction * > (), 0));
                        }
...
/* if( UsingGProxy )
    ...
    if (m_GameLoaded) {
        ...
        if (!m_Lagging) {
            ...
            for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
                if (m_SyncCounter - ( * i) - > GetSyncCounter() > m_SyncLimit) {
                    ...
                    if (LaggingString.empty()) LaggingString = ( * i) - > GetName();
                    else LaggingString += ", " + ( * i) - > GetName();
                }
            }
            ...
            if (m_Lagging) {
                ...
                for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i)( * i) - > SetDropVote(false);
        ...
        if (m_Lagging) {
            ...
            if (UsingGProxy) WaitTime = (m_GProxyEmptyActions + 1) * 60;
...
/* if( UsingGProxy )
    ...
    if (m_GameLoaded && !m_Lagging && GetTicks() - m_LastActionSentTicks >= m_Latency - m_LastActionLateBy) SendAllActions();
    ...
    if (m_Players.empty() && (m_GameLoading || m_GameLoaded)) {
        if (!m_Saving) {
            ...
            m_Saving = true;
            m_GHost - > m_Callables.push_back(m_GHost - > m_DB - > Threadedgs(m_ChatID, string(), 3, uint32_t()));
        } else if (IsGameDataSaved()) return true;
    }
    ...
    if (m_Socket) {
        ...
        if (NewSocket) {
            ...
            if (m_IPBlackList.find(NewSocket - > GetIPString()) == m_IPBlackList.end()) {
                if (m_GHost - > m_TCPNoDelay) NewSocket - > SetNoDelay(true);
        ...
        if (m_Socket - > HasError()) return true;
    }
...
*/
    ...
    if (GetTime() - m_LastPingWarn >= 60 && m_GameLoaded) {
        for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
            ...
            if (( * i) - > GetPing(m_GHost - > m_LCPings) >= 750 && ( * i) - > GetHighPingTimes() > 5)( * i) - > SetHighPingTimes();
        }
        m_LastPingWarn = GetTime();
    }
...
void CBaseGame::SendAll(BYTEARRAY data) {
    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i)( * i) - > Send(data);
}
...
void CBaseGame::SendChat(unsigned char fromPID, CGamePlayer * player, string message) {
    ...
    if (player) {
        if (!m_GameLoading && !m_GameLoaded) {
            if (message.size() > 254) message = message.substr(0, 254);
            ...
            if (SID < m_Slots.size()) ExtraFlags[0] = 3 + m_Slots[SID].GetColour();

            if (message.size() > 127) message = message.substr(0, 127);
...
void CBaseGame::SendAllChat(unsigned char fromPID, string message) {
    ...
    if (GetNumHumanPlayers() > 0) {
        ...
        if (m_GameLoading || m_GameLoaded) {
            ...
            if (message.find("@") == string::npos) GAME_Print(0, "", "", "Bot", "", message);
        }
        if (!m_GameLoading && !m_GameLoaded) {
            if (message.size() > 254) message = message.substr(0, 254);
            ...
            if (m_Replay) m_Replay - > AddChatMessage(fromPID, 32, 0, message);
        }
    }
}
...
void CBaseGame::SendLocalAdminChat(string message) {
    if (!m_LocalAdminMessages) return;
    ...
    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
        if (( * i) - > GetSpoofed() && IsOwner(( * i) - > GetName()) && (UTIL_IsLanIP(( * i) - > GetExternalIP()) || UTIL_IsLocalIP(( * i) - > GetExternalIP(), m_GHost - > m_LocalAddresses))) {
            if (m_VirtualHostPID != 255) SendChat(m_VirtualHostPID, * i, message);
            else {
...
void CBaseGame::SendVirtualHostPlayerInfo(CGamePlayer * player) {
    if (m_VirtualHostPID == 255) return;
...
/* if( UsingGProxy )
    ...
    if (!m_Actions.empty()) {
        ...
        while (!m_Actions.empty()) {
            ...
            if (( * Action - > GetAction())[0] == 0x1) {
                ...
                if (m_FakePlayerPID != 255 && !m_PauseReq) {
                    ...
                    if (Player - > GetPauseTried() == 2) SendChat(Player - > GetPID(), "[PeaceMaker] On the next attemp to pause the game you will be punished");
            ...
            if (SubActionsLength + Action - > GetLength() > 1452) {
                ...
                if (m_Replay) m_Replay - > AddTimeSlot2(SubActions);
        ...
        if (m_Replay) m_Replay - > AddTimeSlot(m_Latency, SubActions);
        ...
        if (m_Replay) m_Replay - > AddTimeSlot(m_Latency, m_Actions);
    }
...
void CBaseGame::SendWelcomeMessage(CGamePlayer * player) {
    ...
    ifstream in ; in .open(m_GHost - > m_MOTDFile.c_str());

    if ( in .fail()) {
        // default welcome message

        if (m_HCLCommandString.empty()) SendChat(player, " ");
        ...
        if (!m_HCLCommandString.empty()) SendChat(player, "     HCL Command String:  " + m_HCLCommandString);
    } else {
        ...
        while (! in .eof() && Count < 8) {
            ...
            if (Line.empty()) {
                if (! in .eof()) SendChat(player, " ");
            } else SendChat(player, Line);
...
void CBaseGame::SendEndMessage() {
    ...
    ifstream in ; in .open(m_GHost - > m_GameOverFile.c_str());
...
void CBaseGame::EventPlayerDeleted(CGamePlayer * player) {
    ...
    if (player - > GetWhoisSent() && !player - > GetJoinedRealm().empty() && player - > GetSpoofedRealm().empty()) {
        for (vector < CBNET * > ::iterator i = m_GHost - > m_BNETs.begin(); i != m_GHost - > m_BNETs.end(); ++i) {
            if (( * i) - > GetServer() == player - > GetJoinedRealm()) {
                ...
                if (( * i) - > GetPasswordHashType() == "pvpgn")( * i) - > UnqueueChatCommand("/whereis " + player - > GetName());
                else( * i) - > UnqueueChatCommand("/whois " + player - > GetName());
    ...
    if (player - > GetLeftMessageSent()) return;
    ...
    if (player - > GetLagging()) SendAll(m_Protocol - > SEND_W3GS_STOP_LAG(player));
    ...
    if (m_GameLoading && m_LoadInGame) {
        ...
        for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
            if (( * i) - > GetFinishedLoading()) {
                if (!player - > GetFinishedLoading()) Send( * i, m_Protocol - > SEND_W3GS_STOP_LAG(player));

                Send( * i, m_Protocol - > SEND_W3GS_PLAYERLEAVE_OTHERS(player - > GetPID(), player - > GetLeftCode()));
            } else( * i) - > AddLoadInGameData(m_Protocol - > SEND_W3GS_PLAYERLEAVE_OTHERS(player - > GetPID(), player - > GetLeftCode()));
        }
    } else {
    ...
    if (m_Replay && (m_GameLoading || m_GameLoaded)) {
        ...
        if (m_GameLoading && !m_LoadInGame) m_Replay - > AddLeaveGameDuringLoading(1, player - > GetPID(), player - > GetLeftCode());
        else m_Replay - > AddLeaveGame(1, player - > GetPID(), player - > GetLeftCode());
    }
    ...
    if (!m_KickVotePlayer.empty()) SendAllChat(m_GHost - > m_Language - > VoteKickCancelled(m_KickVotePlayer));

    if (!m_GameLoading && !m_GameLoaded) m_Denied.push_back(player - > GetName() + " " + player - > GetExternalIPString() + " " + UTIL_ToString(GetTime()));
...
void CBaseGame::EventPlayerDisconnectTimedOut(CGamePlayer * player) {
    if (player - > GetGProxy() && m_GameLoaded) {
        ...
        if (GetTime() - player - > GetLastGProxyWaitNoticeSentTime() >= 20) {
            ...
            if (TimeRemaining > ((uint32_t) m_GProxyEmptyActions + 1) * 60) TimeRemaining = (m_GProxyEmptyActions + 1) * 60;
    ...
    if (GetTime() - m_LastLagScreenTime >= 10) {
        ...
        if (!m_GameLoading && !m_GameLoaded) OpenSlot(GetSIDFromPID(player - > GetPID()), false);
...
void CBaseGame::EventPlayerJoined(CPotentialPlayer * potential, CIncomingJoinPlayer * joinPlayer) {
    ...
    for (vector < CBNET * > ::iterator i = m_GHost - > m_BNETs.begin(); i != m_GHost - > m_BNETs.end(); ++i) {
        if (( * i) - > GetHostCounterID() == HostCounterID) JoinedRealm = ( * i) - > GetServer();
    }
    ...
    if (m_GHost - > m_GarenaHostingOnly) {
        ...
        for (vector < string > ::iterator i = m_GarenaPermissions.begin(); i != m_GarenaPermissions.end(); ++i) {
            ...
            if (username == name) {
                ...
                Level = level;
                if (level == 0) LevelName = "Member";
                else if (level == 1) LevelName = "Vouched";
                else if (level == 2) LevelName = "Reserved";
                else if (level == 3) LevelName = "Safelisted";
                else if (level == 4) LevelName = "Website Moderator";
                else if (level == 5) LevelName = "Simple Bot Moderator";
                else if (level == 6) LevelName = "Full Bot Moderator";
                else if (level == 7) LevelName = "Global Moderator";
                else if (level == 8) LevelName = "Hoster";
    ...
    if (joinPlayer - > GetName().empty() || joinPlayer - > GetName().size() > 15 || LowerName.find(" ") != string::npos || LowerName.find("|") != string::npos) {
        ...
        if (!m_GHost - > m_BNETs.empty()) m_GHost - > m_Callables.push_back(m_GHost - > m_DB - > ThreadedBanAdd(m_GHost - > m_BNETs[0] - > GetServer(), joinPlayer - > GetName(), potential - > GetExternalIPString(), m_GameName, "PeaceMaker", "attempted to join with spoofed name: " + joinPlayer - > GetName(), 0, ""));
    ...
    if (m_GameType == 5) {
        if (!Reserved) {
            ...
            potential - > SetDeleteMe(true);
            return;
        } else CONSOLE_Print("[GAME: " + m_GameName + "] player [" + joinPlayer - > GetName() + "|" + potential - > GetExternalIPString() + "] joined with " + LevelName + " Status.");
    }
    ...
    if (m_SaveGame) {
        ...
        for (vector < PIDPlayer > ::iterator i = m_EnforcePlayers.begin(); i != m_EnforcePlayers.end(); ++i) {
            if (( * i).second == joinPlayer - > GetName()) EnforcePID = ( * i).first;
        }
    ...
    if (GetSlotsAllocated() >= m_Slots.size() - 1 || EnforcePID == m_VirtualHostPID) DeleteVirtualHost();
    ...
    if (JoinedRealm.empty()) Player - > SetSpoofed(true);
    ...
    if (m_SaveGame) m_Slots[SID] = EnforceSlot;
    else {
        if (m_Map - > GetMapOptions() & MAPOPT_CUSTOMFORCES) m_Slots[SID] = CGameSlot(Player - > GetPID(), 255, SLOTSTATUS_OCCUPIED, 0, m_Slots[SID].GetTeam(), m_Slots[SID].GetColour(), m_Slots[SID].GetRace());
        else {
            if (m_Map - > GetMapFlags() & MAPFLAG_RANDOMRACES) m_Slots[SID] = CGameSlot(Player - > GetPID(), 255, SLOTSTATUS_OCCUPIED, 0, 12, 12, SLOTRACE_RANDOM);
            else m_Slots[SID] = CGameSlot(Player - > GetPID(), 255, SLOTSTATUS_OCCUPIED, 0, 12, 12, SLOTRACE_RANDOM | SLOTRACE_SELECTABLE);
            ...
            for (unsigned char i = 0; i < m_Slots.size(); ++i) {
                if (m_Slots[i].GetSlotStatus() == SLOTSTATUS_OCCUPIED && m_Slots[i].GetTeam() != 12) NumOtherPlayers++;
            }

            if (NumOtherPlayers < m_Map - > GetMapNumPlayers()) {
                if (SID < m_Map - > GetMapNumPlayers()) m_Slots[SID].SetTeam(SID);
                else m_Slots[SID].SetTeam(0);
    ...
    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
        if (!( * i) - > GetLeftMessageSent() && * i != Player) {
            ...
            if (( * i) - > GetSocket()) {
                if (m_GHost - > m_HideIPAddresses)( * i) - > Send(m_Protocol - > SEND_W3GS_PLAYERINFO(Player - > GetPID(), Player - > GetName(), BlankIP, BlankIP, GetColoredName(Player - > GetName())));
                else( * i) - > Send(m_Protocol - > SEND_W3GS_PLAYERINFO(Player - > GetPID(), Player - > GetName(), Player - > GetExternalIP(), Player - > GetInternalIP(), GetColoredName(Player - > GetName())));
            }
            // send info about every other player to the new player

            if (m_GHost - > m_HideIPAddresses) Player - > Send(m_Protocol - > SEND_W3GS_PLAYERINFO(( * i) - > GetPID(), ( * i) - > GetName(), BlankIP, BlankIP, GetColoredName(( * i) - > GetName())));
            else Player - > Send(m_Protocol - > SEND_W3GS_PLAYERINFO(( * i) - > GetPID(), ( * i) - > GetName(), ( * i) - > GetExternalIP(), ( * i) - > GetInternalIP(), GetColoredName(( * i) - > GetName())));
        }
    }
    ...
    if (m_GHost - > m_RequireSpoofChecks && !Player - > GetWhoisShouldBeSent()) {
        for (vector < CBNET * > ::iterator i = m_GHost - > m_BNETs.begin(); i != m_GHost - > m_BNETs.end(); ++i) {
            ...
            if (( * i) - > GetServer() == JoinedRealm) SendChat(Player, m_GHost - > m_Language - > SpoofCheckByWhispering(string(UniqueName.begin(), UniqueName.end())));
        }
    }
    ...
    if (m_GHost - > m_CheckMultipleIPUsage) {
        ...
        for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
            if (Player != * i && Player - > GetExternalIPString() == ( * i) - > GetExternalIPString()) {
                if (Others.empty()) Others = ( * i) - > GetName();
                else Others += ", " + ( * i) - > GetName();
            }
        }

        if (!Others.empty()) SendAllChat(m_GHost - > m_Language - > MultipleIPAddressUsageDetected(joinPlayer - > GetName(), Others));
    }
    ...
    // check leaveperc
    if (Player - > GetLeavePerc() >= 60) SendAllChat("User " + Player - > GetName() + " got a huge leaver percentage of " + UTIL_ToString(Player - > GetLeavePerc(), 2) + "%");
}
...
void CBaseGame::EventPlayerJoinedWithScore(CPotentialPlayer * potential, CIncomingJoinPlayer * joinPlayer, double score) {
    ...
    if (SID == 255) {
        ...
        if (m_GHost - > m_MatchMakingMethod == 0) {
            ...
            if (PlayersScored > 0) AverageScore /= PlayersScored;
            ...
            for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
                if (!FurthestPlayer || ( * i) - > GetScore() < -99999.0 || abs(( * i) - > GetScore() - AverageScore) > abs(FurthestPlayer - > GetScore() - AverageScore)) FurthestPlayer = * i;
            }
            ...
            if (score < -99999.0 || abs(score - AverageScore) > abs(FurthestPlayer - > GetScore() - AverageScore)) {
                if (score < -99999.0) CONSOLE_Print("[GAME: " + m_GameName + "] player [" + joinPlayer - > GetName() + "|" + potential - > GetExternalIPString() + "] is trying to join the game but has the furthest rating [N/A] from the average [" + UTIL_ToString(AverageScore, 2) + "]");
                else CONSOLE_Print("[GAME: " + m_GameName + "] player [" + joinPlayer - > GetName() + "|" + potential - > GetExternalIPString() + "] is trying to join the game but has the furthest rating [" + UTIL_ToString(score, 2) + "] from the average [" + UTIL_ToString(AverageScore, 2) + "]");
            ...
            if (FurthestPlayer - > GetScore() < -99999.0) FurthestPlayer - > SetLeftReason(m_GHost - > m_Language - > WasKickedForHavingFurthestScore("N/A", UTIL_ToString(AverageScore, 2)));
            else FurthestPlayer - > SetLeftReason(m_GHost - > m_Language - > WasKickedForHavingFurthestScore(UTIL_ToString(FurthestPlayer - > GetScore(), 2), UTIL_ToString(AverageScore, 2)));
            ...
            if (FurthestPlayer - > GetScore() < -99999.0) SendAllChat(m_GHost - > m_Language - > PlayerWasKickedForFurthestScore(FurthestPlayer - > GetName(), "N/A", UTIL_ToString(AverageScore, 2)));
            else SendAllChat(m_GHost - > m_Language - > PlayerWasKickedForFurthestScore(FurthestPlayer - > GetName(), UTIL_ToString(FurthestPlayer - > GetScore(), 2), UTIL_ToString(AverageScore, 2)));
            ...
            for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
                if (!LowestPlayer || ( * i) - > GetScore() < -99999.0 || ( * i) - > GetScore() < LowestPlayer - > GetScore()) LowestPlayer = * i;
            }
            ...
            if (score < -99999.0 || score < LowestPlayer - > GetScore()) {
                if (score < -99999.0) CONSOLE_Print("[GAME: " + m_GameName + "] player [" + joinPlayer - > GetName() + "|" + potential - > GetExternalIPString() + "] is trying to join the game but has the lowest rating [N/A]");
                else CONSOLE_Print("[GAME: " + m_GameName + "] player [" + joinPlayer - > GetName() + "|" + potential - > GetExternalIPString() + "] is trying to join the game but has the lowest rating [" + UTIL_ToString(score, 2) + "]");
            ...
            if (LowestPlayer - > GetScore() < -99999.0) LowestPlayer - > SetLeftReason(m_GHost - > m_Language - > WasKickedForHavingLowestScore("N/A"));
            else LowestPlayer - > SetLeftReason(m_GHost - > m_Language - > WasKickedForHavingLowestScore(UTIL_ToString(LowestPlayer - > GetScore(), 2)));
            ...
            if (LowestPlayer - > GetScore() < -99999.0) SendAllChat(m_GHost - > m_Language - > PlayerWasKickedForLowestScore(LowestPlayer - > GetName(), "N/A"));
            else SendAllChat(m_GHost - > m_Language - > PlayerWasKickedForLowestScore(LowestPlayer - > GetName(), UTIL_ToString(LowestPlayer - > GetScore(), 2)));
        }
    }
    ...
    if (GetSlotsOccupied() >= m_Slots.size() - 1) DeleteVirtualHost();
    ...
    if (score < -99999.0) SendAllChat(m_GHost - > m_Language - > PlayerHasScore(joinPlayer - > GetName(), "N/A"));
    else SendAllChat(m_GHost - > m_Language - > PlayerHasScore(joinPlayer - > GetName(), UTIL_ToString(score, 2)));
    ...
    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
        if (!( * i) - > GetLeftMessageSent()) {
            if (( * i) - > GetScore() < -99999.0) PlayersNotScored++;
            else {
                PlayersScored++;
                AverageScore += ( * i) - > GetScore();

                if (!Found || ( * i) - > GetScore() < MinScore) MinScore = ( * i) - > GetScore();

                if (!Found || ( * i) - > GetScore() > MaxScore) MaxScore = ( * i) - > GetScore();
    ...
    if (m_AutoStartPlayers != 0 && GetNumHumanPlayers() == m_AutoStartPlayers) BalanceSlots();
...
void CBaseGame::EventPlayerLeft(CGamePlayer * player, uint32_t reason) {
    ...
    if (reason == PLAYERLEAVE_GPROXY) player - > SetLeftReason(m_GHost - > m_Language - > WasUnrecoverablyDroppedFromGProxy());
    else {
...
void CBaseGame::EventPlayerLoaded(CGamePlayer * player) {
    ...
    if (m_LoadInGame) {
        ...
        if (!FinishedLoading) Send(player, m_Protocol - > SEND_W3GS_START_LAG(m_Players, true));

        // remove the new player from previously loaded players' lag screens

        for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
            if ( * i != player && ( * i) - > GetFinishedLoading()) Send( * i, m_Protocol - > SEND_W3GS_STOP_LAG(player));
        }

        // send a chat message to previously loaded players

        for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
            if ( * i != player && ( * i) - > GetFinishedLoading()) SendChat( * i, m_GHost - > m_Language - > PlayerFinishedLoading(player - > GetName()));
        }

        if (!FinishedLoading) SendChat(player, m_GHost - > m_Language - > PleaseWaitPlayersStillLoading());
    } else SendAll(m_Protocol - > SEND_W3GS_GAMELOADED_OTHERS(player - > GetPID()));
}
    ...
    if (!action - > GetAction() - > empty()) {
        ...
        if (PacketLength > 0) {
            ...
            while (n < PacketLength && !Failed) {
                ...
                case 0x16:
                case 0x17:
                    if (n + 4 > PacketLength) Failed = true;
                    else {
                        unsigned char i = ( * ActionData)[n + 2];
                        if (( * ActionData)[n + 3] != 0x00 || i > 16) Failed = true;
                        else n += (4 + (i * 8));
                    }
                    PlayerActivity = true;
                    break;
                ...
                if (ActionSize > 1024) Failed = true;
            }


            if (PlayerActivity) player - > SetTimeActive(GetTime());
        }
    }
...
void CBaseGame::EventPlayerKeepAlive(CGamePlayer * player, uint32_t checkSum) {
    ...
    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
        if (!( * i) - > GetDeleteMe() && ( * i) - > GetCheckSums() - > empty()) return;
    }
    ...
    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
        if (!( * i) - > GetDeleteMe() && ( * i) - > GetCheckSums() - > front() != FirstCheckSum) {
            ...
            for (vector < CGamePlayer * > ::iterator j = m_Players.begin(); j != m_Players.end(); ++j) {
                if (!( * j) - > GetDeleteMe()) Bins[( * j) - > GetCheckSums() - > front()].push_back(( * j) - > GetPID());
            }
            ...
            for (map < uint32_t, vector < unsigned char > > ::iterator j = Bins.begin(); j != Bins.end(); ++j) {
                if (( * j).second.size() > ( * LargestBin).second.size()) {
                    LargestBin = j;
                    Tied = false;
                } else if (j != LargestBin && ( * j).second.size() == ( * LargestBin).second.size()) Tied = true;
                ...
                for (vector < unsigned char > ::iterator k = ( * j).second.begin(); k != ( * j).second.end(); ++k) {
                    ...
                    if (Player) {
                        if (Players.empty()) Players = Player - > GetName();
    ...
    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
        if (!( * i) - > GetDeleteMe())( * i) - > GetCheckSums() - > pop();
    }
        ...
        if (chatPlayer - > GetType() == CIncomingChatPlayer::CTH_MESSAGE || chatPlayer - > GetType() == CIncomingChatPlayer::CTH_MESSAGEEXTRA) {
            ...
            if (!ExtraFlags.empty()) {
                ...
                if (!m_GameLoaded) Relay = false;
                    ...
                    if (fteam == 0) GAME_Print(5, MinString, SecString, player - > GetName(), "", chatPlayer - > GetMessage());
                    else GAME_Print(6, MinString, SecString, player - > GetName(), "", chatPlayer - > GetMessage());
                ...
                if (Relay) {
                    ...
                    if (m_Replay) m_Replay - > AddChatMessage(chatPlayer - > GetFromPID(), chatPlayer - > GetFlag(), UTIL_ByteArrayToUInt32(chatPlayer - > GetExtraFlags(), false), chatPlayer - > GetMessage());
                }
            } else {
                if (m_GameLoading || m_GameLoaded) Relay = false;
                else {
            ...
            if (Message == "?trigger") SendChat(player, m_GHost - > m_Language - > CommandTrigger(string(1, m_GHost - > m_CommandTrigger)));
                ...
                if (EventPlayerBotCommand(player, Command, Payload)) Relay = false;
            }
            ...
            if (Relay) {
                ...
                for (unsigned int i = 0; i < PIDs.size(); ++i) {
                    ...
                    if (toPlayer) {
                        if (!toPlayer - > GetIsIgnoring(player - > GetName()) && !player - > GetIsIgnoring(toPlayer - > GetName())) Send(toPlayer, m_Protocol - > SEND_W3GS_CHAT_FROM_HOST(chatPlayer - > GetFromPID(), Silence(chatPlayer - > GetToPIDs()), chatPlayer - > GetFlag(), chatPlayer - > GetExtraFlags(), chatPlayer - > GetMessage()));
                    }
                }
            }
        } else if (chatPlayer - > GetType() == CIncomingChatPlayer::CTH_TEAMCHANGE && !m_CountDownStarted) EventPlayerChangeTeam(player, chatPlayer - > GetByte());
        else if (chatPlayer - > GetType() == CIncomingChatPlayer::CTH_COLOURCHANGE && !m_CountDownStarted) EventPlayerChangeColour(player, chatPlayer - > GetByte());
        else if (chatPlayer - > GetType() == CIncomingChatPlayer::CTH_RACECHANGE && !m_CountDownStarted) EventPlayerChangeRace(player, chatPlayer - > GetByte());
        else if (chatPlayer - > GetType() == CIncomingChatPlayer::CTH_HANDICAPCHANGE && !m_CountDownStarted) EventPlayerChangeHandicap(player, chatPlayer - > GetByte());
    }
}
...
void CBaseGame::EventPlayerChangeTeam(CGamePlayer * player, unsigned char team) {
    ...
    if (m_Map - > GetMapOptions() & MAPOPT_CUSTOMFORCES) {
        ...
        if (team == 12) {
            if (m_Map - > GetMapObservers() != MAPOBS_ALLOWED && m_Map - > GetMapObservers() != MAPOBS_REFEREES) return;
        } else {
            if (team >= m_Map - > GetMapNumPlayers()) return;
            ...
            for (unsigned char i = 0; i < m_Slots.size(); ++i) {
                if (m_Slots[i].GetSlotStatus() == SLOTSTATUS_OCCUPIED && m_Slots[i].GetTeam() != 12 && m_Slots[i].GetPID() != player - > GetPID())++NumOtherPlayers;
            }

            if (NumOtherPlayers >= m_Map - > GetMapNumPlayers()) return;
        }
...
void CBaseGame::EventPlayerChangeColour(CGamePlayer * player, unsigned char colour) {
    ...
    if (m_Map - > GetMapOptions() & MAPOPT_FIXEDPLAYERSETTINGS) return;
    ...
    if (SID < m_Slots.size()) {
        ...
        if (m_Slots[SID].GetTeam() == 12) return;
...
void CBaseGame::EventPlayerChangeRace(CGamePlayer * player, unsigned char race) {
    ...
    if (m_Map - > GetMapFlags() & MAPFLAG_RANDOMRACES) return;

    if (race != SLOTRACE_HUMAN && race != SLOTRACE_ORC && race != SLOTRACE_NIGHTELF && race != SLOTRACE_UNDEAD && race != SLOTRACE_RANDOM) return;
...
void CBaseGame::EventPlayerChangeHandicap(CGamePlayer * player, unsigned char handicap) {
    ...
    if (handicap != 50 && handicap != 60 && handicap != 70 && handicap != 80 && handicap != 90 && handicap != 100) return;
...
void CBaseGame::EventPlayerMapSize(CGamePlayer * player, CIncomingMapSize * mapSize) {
    if (m_GameLoading || m_GameLoaded) return;
        ...
        if (m_GHost - > m_AllowDownloads != 0) {
            ...
            if (!MapData - > empty()) {
                if (m_GHost - > m_AllowDownloads == 1 || (m_GHost - > m_AllowDownloads == 2 && player - > GetDownloadAllowed())) {
                    ...
                    } else player - > SetLastMapPartAcked(mapSize - > GetMapSize());
                }
            } else {
                player - > SetDeleteMe(true);
    ...
    if (NewDownloadStatus > 100) NewDownloadStatus = 100;
...
void CBaseGame::EventGameStarted() {
        ...
        if (m_HCLCommandString.size() <= GetSlotsOccupied()) {
            ...
            if (m_HCLCommandString.find_first_not_of(HCLChars) == string::npos) {
                ...
                for (uint32_t i = 0; i < 256; ++i) {
                    ...
                    if (j == 0 || j == 50 || j == 60 || j == 70 || j == 80 || j == 90 || j == 100)++j;
                ...
                for (string::iterator si = m_HCLCommandString.begin(); si != m_HCLCommandString.end(); ++si) {
                    while (m_Slots[CurrentSlot].GetSlotStatus() != SLOTSTATUS_OCCUPIED)++CurrentSlot;
            ...
            } else CONSOLE_Print("[GAME: " + m_GameName + "] encoding HCL command string [" + m_HCLCommandString + "] failed because it contains invalid characters");
        } else CONSOLE_Print("[GAME: " + m_GameName + "] encoding HCL command string [" + m_HCLCommandString + "] failed because there aren't enough occupied slots");
    }
    ...
    if (m_FakePlayerPID != 255) SendAll(m_Protocol - > SEND_W3GS_GAMELOADED_OTHERS(m_FakePlayerPID));
    ...
    if (m_Replay) {
        ...
        if (m_FakePlayerPID != 255) m_Replay - > AddPlayer(m_FakePlayerPID, "FakePlayer");
        ...
        if (m_SaveGame) {
            ...
            if (m_GameState == GAME_PRIVATE) MapGameType |= MAPGAMETYPE_PRIVATEGAME;
    ...
    if (m_LoadInGame) {
        ...
        for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
            for (vector < CGamePlayer * > ::iterator j = m_Players.begin(); j != m_Players.end(); ++j)( * j) - > AddLoadInGameData(m_Protocol - > SEND_W3GS_GAMELOADED_OTHERS(( * i) - > GetPID()));
        }
    }
...
void CBaseGame::EventGameLoaded() {
    ...
    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
        if (!Shortest || ( * i) - > GetFinishedLoadingTicks() < Shortest - > GetFinishedLoadingTicks()) Shortest = * i;

        if (!Longest || ( * i) - > GetFinishedLoadingTicks() > Longest - > GetFinishedLoadingTicks()) Longest = * i;
    }
...
unsigned char CBaseGame::GetSIDFromPID(unsigned char PID) {
    if (m_Slots.size() > 255) return 255;

    for (unsigned char i = 0; i < m_Slots.size(); ++i) {
        if (m_Slots[i].GetPID() == PID) return i;
    }
...
CGamePlayer * CBaseGame::GetPlayerFromPID(unsigned char PID) {
    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
        if (!( * i) - > GetLeftMessageSent() && ( * i) - > GetPID() == PID) return *i;
    }
...
CGamePlayer * CBaseGame::GetPlayerFromSID(unsigned char SID) {
    if (SID < m_Slots.size()) return GetPlayerFromPID(m_Slots[SID].GetPID());

    return NULL;
}

CGamePlayer * CBaseGame::GetPlayerFromName(string name, bool sensitive) {
    if (!sensitive) transform(name.begin(), name.end(), name.begin(), (int( * )(int)) tolower);

    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
        if (!( * i) - > GetLeftMessageSent()) {
            string TestName = ( * i) - > GetName();

            if (!sensitive) transform(TestName.begin(), TestName.end(), TestName.begin(), (int( * )(int)) tolower);

            if (TestName == name) return *i;
        }
    }
...
uint32_t CBaseGame::GetPlayerFromNamePartial(string name, CGamePlayer * * player) {
    transform(name.begin(), name.end(), name.begin(), (int( * )(int)) tolower);
    uint32_t Matches = 0; * player = NULL;
...
CGamePlayer * CBaseGame::GetPlayerFromColour(unsigned char colour) {
    for (unsigned char i = 0; i < m_Slots.size(); ++i) {
        if (m_Slots[i].GetColour() == colour) return GetPlayerFromSID(i);
    }
...
string CBaseGame::GetPlayerList() {
    ...
    for (unsigned char i = 0; i < m_Slots.size(); ++i) {
        if (m_Slots[i].GetSlotStatus() == SLOTSTATUS_OCCUPIED && m_Slots[i].GetComputer() == 0) {
            ...
            if (player) players += player - > GetName() + "\t" + player - > GetSpoofedRealm() + "\t" + UTIL_ToString(player - > GetPing(m_GHost - > m_LCPings)) + "\t";
        } else if (m_Slots[i].GetSlotStatus() == SLOTSTATUS_OPEN) players += "\t\t\t";
    }
...
unsigned char CBaseGame::GetNewPID() {
    ...
    for (unsigned char TestPID = 1; TestPID < 255; ++TestPID) {
        if (TestPID == m_VirtualHostPID || TestPID == m_FakePlayerPID) continue;
...
BYTEARRAY CBaseGame::Silence(BYTEARRAY PIDs) {
    ...
    for (uint32_t j = 0; j < PIDs.size(); j++) {
        ...
        if (p) if (!p - > GetSilence()) result.push_back(PIDs[j]);
        if (!p) result.push_back(PIDs[j]);
    }
    return result;
}
...
BYTEARRAY CBaseGame::GetPIDs() {
    ...
    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
        if (!( * i) - > GetLeftMessageSent()) result.push_back(( * i) - > GetPID());
    }
...
BYTEARRAY CBaseGame::GetPIDs(unsigned char excludePID) {
    ...
    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
        if (!( * i) - > GetLeftMessageSent() && ( * i) - > GetPID() != excludePID) result.push_back(( * i) - > GetPID());
    }
...
unsigned char CBaseGame::GetHostPID() {
    ...
    if (m_VirtualHostPID != 255) return m_VirtualHostPID;

    // try to find the fakeplayer next

    if (m_FakePlayerPID != 255) return m_FakePlayerPID;

    // try to find the owner player next

    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
        if (!( * i) - > GetLeftMessageSent() && IsOwner(( * i) - > GetName())) return ( * i) - > GetPID();
    }

    // okay then, just use the first available player

    for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
        if (!( * i) - > GetLeftMessageSent()) return ( * i) - > GetPID();
    }
...
unsigned char CBaseGame::GetEmptySlot(bool reserved) {
    ...
    if (m_SaveGame) {
        ...
        for (unsigned char i = 0; i < m_Slots.size(); ++i) {
            if (m_Slots[i].GetSlotStatus() == SLOTSTATUS_OPEN && SaveGameSlots[i].GetSlotStatus() == SLOTSTATUS_OCCUPIED && SaveGameSlots[i].GetComputer() == 0) return i;
        }
        ...
        for (unsigned char i = 0; i < m_Slots.size(); ++i) {
            if (m_Slots[i].GetSlotStatus() == SLOTSTATUS_OPEN) return i;
        }
...
*/
            ...
            if (LeastSID != 255) return LeastSID;
            ...
            for (unsigned char i = 0; i < m_Slots.size(); ++i) {
                ...
                if (Player && !Player - > GetReserved()) return i;
            }
        }
    }
    ...
    if (StartSlot < m_Slots.size()) {
        ...
        if (m_SaveGame) {
            ...
            for (unsigned char i = StartSlot; i < m_Slots.size(); ++i) {
                if (m_Slots[i].GetSlotStatus() == SLOTSTATUS_OPEN && m_Slots[i].GetTeam() == team && SaveGameSlots[i].GetSlotStatus() == SLOTSTATUS_OCCUPIED && SaveGameSlots[i].GetComputer() == 0) return i;
            }
            ...
            for (unsigned char i = StartSlot; i < m_Slots.size(); ++i) {
                if (m_Slots[i].GetSlotStatus() == SLOTSTATUS_OPEN && m_Slots[i].GetTeam() == team) return i;
            }
    ...
    for (vector < CGameSlot > ::iterator i = m_Slots.begin(); i != m_Slots.end(); ++i) {
        ...
        for (vector < CGamePlayer * > ::iterator x = m_Players.begin(); x != m_Players.end(); ++x) {
            if (GetSIDFromPID(( * x) - > GetPID()) == GetSIDFromPID(( * i).GetPID())) {
                if (( * x) - > GetLocked()) lockedplayer = true;
            }
        }
        if (!lockedplayer) {
            if (( * i).GetSlotStatus() == SLOTSTATUS_OCCUPIED && ( * i).GetComputer() == 0 && ( * i).GetTeam() != 12) PlayerSlots.push_back( * i);
        }
    }
...
void CBaseGame::OHFixedBalance() {
                            ...
                            if (x < v) {
                                for (vector < double > ::iterator e = PlayerWins.begin(); e != PlayerWins.end(); ++e) {
                                    if (v < w) {
                                        ...
                                        if (result > 50) CurrentSpread = result - 50;
                                        else if (result < 50) CurrentSpread = 50 - result;
    ...
    if (!BestOrder.empty()) {
        ...
        for (vector < CGamePlayer * > ::iterator i = m_Players.begin(); i != m_Players.end(); ++i) {
            if (m_Slots[GetSIDFromPID(( * i) - > GetPID())].GetTeam() != 12) {
                if (( * i) - > GetLocked())++m_LockedPlayers;
    ...
    } else SendAllChat("Error, too few slots token");
}
...
void CBaseGame::BalanceSlots() {
        ...
        if (PID < 13) {
            ...
            if (SID < m_Slots.size()) {
                ...
                if (Team < 12) {
                    ...
                    if (( * i) - > GetGames() > 10) double Score = ( * i) - > GetWinPerc();
                    //double Score = (*i)->GetScore( );

                    if (Score < -99999.0) Score = m_Map - > GetMapDefaultPlayerScore();
    ...
    for (unsigned char i = 0; i < 12; ++i) {
        if (TeamSizes[i] > 0) {
            if (AlgorithmCost == 0) AlgorithmCost = nCr(PlayersLeft, TeamSizes[i]);
            else AlgorithmCost *= nCr(PlayersLeft, TeamSizes[i]);
    ...
    for (unsigned char i = 0; i < 12; ++i) {
        ...
        for (unsigned char j = 0; j < TeamSizes[i]; ++j) {
            while (CurrentSlot < m_Slots.size() && m_Slots[CurrentSlot].GetTeam() != i)++CurrentSlot;
